...
import { listPages } from './pancake-client.js';
...
export async function pancakeRoutes(app) {
    ...
    app.post('/api/v1/pancake/config', async (request, reply) => {
        const { orgId } = getAuth(request);
    ...
    app.get('/api/v1/pancake/pages', async (request, reply) => {
        ...
        const connectedPageIds = new Set(connectedAccounts
    ...
    app.post('/api/v1/pancake/connect', async (request, reply) => {
    ...
    // ── Disconnect a page ─────────────────────────────────────────────
    app.delete('/api/v1/pancake/disconnect/:id', async (request, reply) => {
        const { orgId } = getAuth(request);
     ...
     */
    app.post('/api/v1/pancake/sync/:id', async (request, reply) => {
        const { orgId } = getAuth(request);
        const { id } = request.params;
    ...
    // ── Connection status ─────────────────────────────────────────────
    app.get('/api/v1/pancake/status/:id', async (request, reply) => {
        const { orgId } = getAuth(request);
        const { id } = request.params;
        const account = await prisma.channelAccount.findFirst({
            where: { id, orgId },
...
//# sourceMappingURL=pancake-routes.js.map