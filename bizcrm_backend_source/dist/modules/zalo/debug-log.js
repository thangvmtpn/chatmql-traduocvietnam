...
export function truncateData(data) {
    ...
    if (json === undefined)
        return { data: undefined, truncated: false };
...
function sweepIdleBuffers(now) {
    if (now - lastSweepAt < SWEEP_INTERVAL_MS)
        return;
    lastSweepAt = now;
    for (const [key, buf] of buffers) {
        if (now - buf.lastPushAt > ORG_BUFFER_TTL_MS)
            buffers.delete(key);
    }
}
...
export function recordDebug(input, now = Date.now()) {
    ...
    if (!buf) {
        if (key !== UNATTRIBUTED && buffers.size >= MAX_ORG_BUFFERS)
            evictLeastRecentBuffer();
...
export function getDebugLog(orgId) {
