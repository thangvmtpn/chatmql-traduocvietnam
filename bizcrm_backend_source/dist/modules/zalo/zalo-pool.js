...
const debugLog = [];
export function pushDebug(accountId, event, summary, data) {
    const entry = {
...
// ─── Pool API ──────────────────────────────────────────────────────────────────
export async function getPoolStatus(orgId) {
...
export function getPoolEntry(accountId) {
 ...
 */
export async function loginWithQR(accountId, orgId) {
    let currentSessionData = null;
    let scannedDisplayName;
    let scannedAvatar;
    ...
    try {
        ...
        const displayName = scannedDisplayName || ctx?.name || ctx?.displayName || undefined;
        ...
        syncOnConnect(finalAccountId, orgId, api).catch(err => logger.error(`[zalo-pool] syncOnConnect error:`, err.message));
    }
    catch (err) {
...
export async function disconnectAccount(accountId) {
    ...
    if (entry) {
        ...
        if (entry.api) {
            ...
            try {
                entry.api.listener.removeAllListeners();
            }
            catch { }
        }
...
export async function updateStatus(accountId, status) {
    const existing = pool.get(accountId);
 ...
 */
export async function reconnectWithSession(accountId, orgId, session) {
    ...
    try {
        ...
        syncOnConnect(accountId, orgId, api).catch(err => logger.error(`[zalo-pool] syncOnConnect error:`, err.message));
    }
    catch (err) {
 ...
 */
export async function autoReconnectSavedAccounts() {
    ...
    try {
        ...
        for (const account of accounts) {
            ...
            reconnectWithSession(account.id, account.orgId, session).catch(err => logger.error(`[zalo-pool] Auto-reconnect failed for ${account.displayName || account.id}:`, err.message));
...
async function runWatchdogTick() {
    try {
        ...
        for (const acc of accounts) {
            ...
            if (fails >= MAX_AUTO_RECONNECT_ATTEMPTS)
                continue; // give up — needs manual/QR re-login
 ...
 */
export async function sendTypingViaPool(accountId, targetUid, threadType) {
    const entry = pool.get(accountId);
    if (!entry?.api || entry.status !== 'connected')
        return;
    try {
        const { ThreadType } = await import('zca-js');
        await entry.api.sendTypingEvent(targetUid, threadType === 'group' ? ThreadType.Group : ThreadType.User);
    }
    catch {
        // typing là phụ trợ — bỏ qua mọi lỗi
    }
}
export async function sendViaPool(accountId, targetUid, text, _conversationId, quote, threadType) {
    const entry = pool.get(accountId);
    if (entry?.api && entry.status === 'connected') {
        try {
            // Send via zca-js API
            ...
            await entry.api.sendMessage(payload, targetUid, threadTypeEnum);
...
export async function undoViaPool(accountId, targetUid, msgId, cliMsgId, threadType) {
    const entry = pool.get(accountId);
    if (entry?.api && entry.status === 'connected') {
        try {
            const { ThreadType } = await import('zca-js');
 ...
 */
export async function sendImageViaPool(accountId, targetUid, imageBuffer, filename, text, threadType = 0) {
    const entry = pool.get(accountId);
    if (entry?.api && entry.status === 'connected') {
        try {
            const { ThreadType } = await import('zca-js');
            ...
            await entry.api.sendMessage({ msg: text || '', attachments: [attachment] }, targetUid, threadType);
 ...
 * @returns true if sent via Zalo
 */
export async function sendFileViaPool(accountId, targetUid, fileBuffer, filename, text) {
    const entry = pool.get(accountId);
    if (entry?.api && entry.status === 'connected') {
        try {
            const { ThreadType } = await import('zca-js');
            ...
            await entry.api.sendMessage({ msg: text || '', attachments: [attachment] }, targetUid, ThreadType.User);
 ...
 */
export async function syncOnConnect(accountId, orgId, api) {
    const io = getIO();
    let syncedFriends = 0;
    let syncedConvs = 0;
    // ── 1. Sync friend list ─────────────────────────────────────────────
