...
export async function facebookPageTokenImportRoutes(app) {
    ...
    app.post('/api/v1/facebook-page/token-import/pages', { preHandler: authMiddleware }, async (request, reply) => {
        ...
        const session = { orgId: user.orgId, userId: user.id, pages };
    ...
    app.post('/api/v1/facebook-page/token-import/confirm', { preHandler: authMiddleware }, async (request, reply) => {
        if (!isTokenImportEnabled()) {
            return reply.code(403).send({ error: 'fb_token_import_disabled', message: 'Tính năng nhập token đang tắt' });
        }
        const user = request.user;
...
//# sourceMappingURL=token-import-routes.js.map