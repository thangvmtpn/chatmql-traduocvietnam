...
function isEnabled() {
    if (process.env.FACEBOOK_TOKEN_IMPORT_ENABLED === 'false' || process.env.FB_TOKEN_IMPORT === 'false')
        return false;
    if (process.env.FACEBOOK_TOKEN_IMPORT_ENABLED === 'true' || process.env.FB_TOKEN_IMPORT === 'true')
        return true;
    return true;
}
export async function fbTokenImportRoutes(app) {
    ...
    app.post('/api/v1/facebook-page/token-import/pages', async (request, reply) => {
        if (!isEnabled())
            return reply.code(403).send({ error: 'Token import is disabled' });
        const { userToken } = request.body;
        if (!userToken?.trim())
            return reply.code(400).send({ error: 'userToken is required' });
        try {
    ...
    app.post('/api/v1/facebook-page/token-import/confirm', async (request, reply) => {
        if (!isEnabled())
            return reply.code(403).send({ error: 'Token import is disabled' });
...
//# sourceMappingURL=fb-token-import-routes.js.map