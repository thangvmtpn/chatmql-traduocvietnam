...
export class FbApiError extends Error {
    ...
    subcode;
    raw;
    constructor(code, msg, subcode, raw) {
...
export class FbClient {
     ...
     */
    async sendAttachment(psid, buffer, filename, mimeType, type) {
...
export async function exchangeCodeForToken(opts) {
    ...
    const data = await request('GET', `${GRAPH_BASE}/${graphVersion()}/oauth/access_token?${params.toString()}`);
...
export async function getPages(userToken) {
    const data = await request('GET', `${GRAPH_BASE}/${graphVersion()}/me/accounts?fields=id,name,access_token,picture{url}`, userToken);
...
export async function subscribePageWebhook(pageId, pageToken) {
    const params = new URLSearchParams({
        subscribed_fields: 'messages,messaging_postbacks,message_deliveries,message_reads,messaging_optins',
    });
    await request('POST', `${GRAPH_BASE}/${graphVersion()}/${encodeURIComponent(pageId)}/subscribed_apps?${params.toString()}`, pageToken);
}
// ─── Shared HTTP ────────────────────────────────────────────────────────
...
//# sourceMappingURL=fb-client.js.map