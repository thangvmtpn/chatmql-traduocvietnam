...
import { buildAuthorizeUrl, exchangeCodeForToken, exchangeForLongLivedToken, getPages, subscribePageWebhook, FbApiError, } from './fb-client.js';
...
async function trySubscribeWebhook(pageId, pageToken) {
...
export async function facebookPageOAuthRoutes(app) {
    // ─── authed: start ─────────────────────────────────────────────────────────
    app.get('/api/v1/facebook-page/connect/start', { preHandler: authMiddleware }, async (request, reply) => {
        ...
        await redisConnection.set(`fb:oauth:state:${state}`, JSON.stringify({ orgId: user.orgId, userId: user.id }), 'EX', STATE_TTL_SEC);
        const url = buildAuthorizeUrl({ appId, redirectUri, state, scopes: SCOPES });
        return { url };
    });
    // ─── public: callback ──────────────────────────────────────────────────────
    app.get('/api/v1/facebook-page/callback', async (request, reply) => {
        ...
        let appId, appSecret, redirectUri;
        try {
        ...
        try {
            ...
            if (conflicts > 0)
                q.set('fb_conflict', String(conflicts));
