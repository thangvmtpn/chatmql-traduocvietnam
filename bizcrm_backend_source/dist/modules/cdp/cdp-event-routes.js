...
export async function cdpEventRoutes(app) {
    app.addHook('preHandler', authMiddleware);
    // ── TRACK event (internal — JWT auth) ───────────────────────────
    app.post('/api/v1/cdp/events', async (request, reply) => {
    ...
    // ── BATCH TRACK events ──────────────────────────────────────────
    app.post('/api/v1/cdp/events/batch', async (request, reply) => {
        const user = request.user;
        const { events } = request.body;
        if (!events?.length)
            return badRequest(reply, 'events array is required');
        if (events.length > 100)
            return badRequest(reply, 'Max 100 events per batch');
        ...
        for (const def of definitions) {
            if (def.schema) {
                try {
                    defMap.set(def.eventName, ajv.compile(def.schema));
                }
                catch (err) {
    ...
    app.get('/api/v1/cdp/events', async (request) => {
        const user = request.user;
    ...
    app.get('/api/v1/contacts/:contactId/events', async (request, reply) => {
        const user = request.user;
    ...
    app.get('/api/v1/cdp/events/stats', async (request) => {
        const user = request.user;
        const { contactId, days = '30' } = request.query;
        const since = new Date();
        since.setDate(since.getDate() - parseInt(days));
        const where = { orgId: user.orgId, timestamp: { gte: since } };
        if (contactId)
            where.contactId = contactId;
    ...
    app.get('/api/v1/contacts/:contactId/timeline', async (request, reply) => {
        const user = request.user;
        const { contactId } = request.params;
...
//# sourceMappingURL=cdp-event-routes.js.map