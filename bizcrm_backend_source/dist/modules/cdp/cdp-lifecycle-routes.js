...
export async function cdpLifecycleRoutes(app) {
    app.addHook('preHandler', authMiddleware);
    // GET lifecycle history + current stage
    app.get('/api/v1/contacts/:contactId/lifecycle', async (request, reply) => {
    ...
    app.post('/api/v1/contacts/:contactId/lifecycle', async (request, reply) => {
        const user = request.user;
        const { contactId } = request.params;
        const { toStage, reason } = request.body;
        if (!toStage?.trim())
            return badRequest(reply, 'toStage is required');
        try {
...
//# sourceMappingURL=cdp-lifecycle-routes.js.map