...
export async function cdpPresetRoutes(app) {
    ...
    // ── GET preset detail ───────────────────────────────────────────
    app.get('/api/v1/cdp/presets/:key', async (request, reply) => {
    ...
    // ── APPLY a preset ─────────────────────────────────────────────
    app.post('/api/v1/cdp/presets/:key/apply', async (request, reply) => {
        const { orgId } = request.user;
        const preset = PRESET_MAP.get(request.params.key);
        if (!preset)
            return notFound(reply, 'Preset');
        ...
        const result = await prisma.$transaction(async (tx) => {
            ...
            for (const p of propsToApply) {
                if (propKeySet.has(p.fieldKey)) {
                    counts.skipped.props++;
                    continue;
                }
            ...
            for (const e of eventsToApply) {
                if (eventNameSet.has(e.eventName)) {
                    counts.skipped.events++;
                    continue;
                }
            ...
            for (const auto of autoToApply) {
                ...
                const actions = auto.actions.map(a => a.params.templateId === '__preset__' && templateId ? { ...a, params: { ...a.params, templateId } } : a);
    ...
    // ── UNINSTALL a preset ──────────────────────────────────────────
    app.delete('/api/v1/cdp/presets/:key/uninstall', async (request, reply) => {
        const { orgId } = request.user;
        const preset = PRESET_MAP.get(request.params.key);
        if (!preset)
            return notFound(reply, 'Preset');
...
//# sourceMappingURL=cdp-preset-routes.js.map