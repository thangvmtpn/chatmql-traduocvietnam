...
const lastForwardAt = new Map();
/** Throttled, best-effort forward of a staff typing signal to Zalo personal. */
export async function forwardStaffTypingToZalo(orgId, convId) {
    const now = Date.now();
    const last = lastForwardAt.get(convId) ?? 0;
    if (now - last < THROTTLE_MS)
        return;
    lastForwardAt.set(convId, now);
    if (lastForwardAt.size > MAX_TRACKED_CONVS)
        lastForwardAt.clear(); // bounded memory
    ...
    if (conv?.channelAccount?.platform !== Platform.ZALO_USER)
        return;
    const recipient = conv.threadType === 'group' ? conv.externalThreadId : conv.contact?.zaloUid;
    if (!recipient)
        return;
    await sendTypingViaPool(conv.channelAccountId, recipient, conv.threadType);
}
//# sourceMappingURL=typing-forward.js.map