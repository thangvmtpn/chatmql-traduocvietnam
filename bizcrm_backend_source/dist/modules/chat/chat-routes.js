...
import { emitReaction, emitAiModeChanged, emitConvDeleted } from '../realtime/socket-gateway.js';
...
export function buildReplyQuote(message) {
    if (!message.externalMsgId || !message.senderUid)
        return null;
    return {
 ...
 */
export function transformMessageForFrontend(message) {
    const { quote, ...rest } = message;
    const actorKind = deriveActorKind(message);
    ...
    rest.content = safeContent;
    const q = quote;
    if (!q || typeof q !== 'object')
        return { ...rest, reply: null, actorKind };
    return {
        ...rest,
        actorKind,
        reply: {
            msgId: q.msgId || q.cliMsgId || '',
            cliMsgId: q.cliMsgId || undefined,
            content: q.content || '',
            msgType: q.msgType || 'text',
            uidFrom: q.uidFrom || '',
            ts: q.ts || '',
        },
    };
...
export async function chatRoutes(app) {
    ...
    app.get('/api/v1/conversations/counts', async (request) => {
        ...
        const { accountId = '', tab = '' } = request.query;
        const baseWhere = {
            orgId: user.orgId,
            channelAccount: {
                isDisabled: false,
    ...
    app.get('/api/v1/conversations', async (request) => {
        ...
        if (from || to) {
            ...
            if (to) {
                const d = new Date(to);
                if (!isNaN(d.getTime()))
                    where.lastMessageAt.lte = d;
            }
            if (Object.keys(where.lastMessageAt).length === 0)
    ...
    app.get('/api/v1/conversations/:id', { preHandler: requireZaloAccess() }, async (request, reply) => {
        const user = request.user;
    ...
    app.get('/api/v1/conversations/:id/messages', { preHandler: requireZaloAccess() }, async (request, reply) => {
        const user = request.user;
    ...
    app.post('/api/v1/conversations/:id/messages/:msgId/reaction', { preHandler: requireZaloAccess() }, async (request, reply) => {
        const user = request.user;
        ...
        if (!message)
            return reply.status(404).send({ error: 'Không tìm thấy tin nhắn trong hội thoại này' });
        ...
        const action = emoji === '' ? 'removed' : 'added';
        if (action === 'added') {
    ...
    app.patch('/api/v1/conversations/:id/ai-mode', async (request, reply) => {
        const user = request.user;
    ...
    app.patch('/api/v1/conversations/:id/ai-pause', async (request, reply) => {
        const user = request.user;
    ...
    app.post('/api/v1/conversations/:id/backfill', { preHandler: requireZaloAccess() }, async (request, reply) => {
        const user = request.user;
        ...
        const result = await backfillConversation(poolEntry.api, conv.channelAccountId, conv.externalThreadId, threadType, maxMessages);
        return {
            success: true,
            convId: conv.id,
