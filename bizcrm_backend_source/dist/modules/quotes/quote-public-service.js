...
export function isOrgActive(org) {
    ...
    if (org.status !== 'active')
        return false;
    if (org.expiresAt != null && org.expiresAt.getTime() < Date.now())
        return false;
    return true;
}
const PUBLIC_SELECT = {
 ...
 */
export async function getPublicQuote(token, meta = {}) {
    ...
    if (!isOrgActive(quote.org))
        return { kind: 'not_found' };
    // Đã thu hồi hoặc huỷ → báo rõ, không hiện giá
    if (quote.deletedAt != null || quote.status === 'canceled')
        return { kind: 'gone', reason: 'canceled' };
...
export async function respondToPublicQuote(token, action, meta = {}) {
    if (!token || token.length < 20 || token.length > 100)
        return { kind: 'not_found' };
    const quote = await prisma.quote.findFirst({
        where: { publicToken: token },
    ...
    if (quote.deletedAt != null)
        return { kind: 'conflict', message: 'Báo giá này đã được thu hồi' };
    if (quote.respondedAt != null)
        return { kind: 'conflict', message: 'Báo giá này đã được phản hồi trước đó' };
...
//# sourceMappingURL=quote-public-service.js.map