...
const daysRemaining = (now, until) => Math.max(1, Math.ceil((until.getTime() - now.getTime()) / DAY_MS));
/** Số ngày ĐÃ TRÔI QUA — làm tròn XUỐNG (gửi 9,2 ngày trước = "9 ngày trước"). */
const daysElapsed = (since, now) => Math.max(1, Math.floor((now.getTime() - since.getTime()) / DAY_MS));
/** Đã nhắc loại này cho báo giá này chưa? QuoteEvent đóng vai trò cờ. */
async function alreadyReminded(quoteId, kind) {
...
async function notify(quote, kind, title, body) {
...
const customerName = (q) => q.contact?.crmName || q.contact?.fullName || 'Khách hàng';
/** Báo giá sắp hết hiệu lực mà khách chưa phản hồi. */
export async function remindExpiringQuotes(now = new Date()) {
    ...
    for (const q of rows) {
        try {
            if (await alreadyReminded(q.id, 'reminded_expiring'))
                continue;
            const days = q.validUntil ? daysRemaining(now, q.validUntil) : 1;
            const ok = await notify(q, 'reminded_expiring', 'Báo giá sắp hết hiệu lực', `${q.number} cho ${customerName(q)} (${formatMoney(Number(q.total))}đ) còn ${days} ngày — nên gọi chốt.`);
            if (ok)
                sent++;
        }
        catch (err) {
...
export async function remindUnviewedQuotes(now = new Date()) {
    ...
    for (const q of rows) {
        try {
            if (await alreadyReminded(q.id, 'reminded_unviewed'))
                continue;
            const ok = await notify(q, 'reminded_unviewed', 'Khách chưa mở báo giá', 
...
//# sourceMappingURL=quote-reminder-service.js.map