...
import { canViewAllQuotes, discountCeilingFor, } from './quote-types.js';
const LINE_SELECT = {
...
export class QuoteForbiddenError extends Error {
    code = 'FORBIDDEN';
    constructor(message) { super(message); this.name = 'QuoteForbiddenError'; }
}
export class QuoteValidationError extends Error {
    code = 'VALIDATION_ERROR';
    constructor(message) { super(message); this.name = 'QuoteValidationError'; }
}
function newPublicToken() {
    return randomBytes(24).toString('base64url');
}
/** Ghi nhật ký — không bao giờ làm hỏng luồng chính. */
async function logEvent(tx, input) {
...
function buildLines(lines) {
    ...
    return lines.map((l, i) => {
        ...
        if (!name)
            throw new QuoteValidationError(`Dòng ${i + 1}: thiếu tên hàng hoá/dịch vụ`);
        const quantity = Number(l.quantity);
        const unitPrice = Number(l.unitPrice);
        if (!Number.isFinite(quantity) || quantity <= 0)
            throw new QuoteValidationError(`Dòng ${i + 1}: số lượng không hợp lệ`);
        if (!Number.isFinite(unitPrice) || unitPrice < 0)
            throw new QuoteValidationError(`Dòng ${i + 1}: đơn giá không hợp lệ`);
...
function assertDiscountAllowed(role, lines, opts) {
    const ceiling = discountCeilingFor(role);
    if (ceiling >= 100)
        return;
    const effective = effectiveDiscountPercent(lines.map((l) => ({ quantity: Number(l.quantity), unitPrice: Number(l.unitPrice), discountPercent: Number(l.discountPercent ?? 0) })), { discountType: opts.discountType, discountValue: opts.discountValue });
...
async function assertContactInOrg(orgId, contactId) {
    const contact = await prisma.contact.findFirst({
        where: { id: contactId, orgId, deletedAt: null },
        select: { id: true, companyId: true },
    });
    if (!contact)
        throw new QuoteValidationError('Không tìm thấy khách hàng trong tổ chức này');
    return { companyId: contact.companyId };
}
/** Xác nhận company thuộc org này — chặn gắn báo giá vào công ty của org khác. */
async function assertCompanyInOrg(orgId, companyId) {
    const company = await prisma.company.findFirst({
        where: { id: companyId, orgId, deletedAt: null },
        select: { id: true },
    });
    if (!company)
        throw new QuoteValidationError('Công ty không thuộc tổ chức này');
}
// ── Đọc ──────────────────────────────────────────────────────────────
export async function listQuotes(orgId, user, filters = {}, page = { page: 1, limit: 20 }) {
...
export async function getQuote(orgId, id, user) {
...
export async function getQuoteEvents(orgId, id) {
...
export async function createQuote(orgId, user, input) {
    ...
    if (input.companyId)
        await assertCompanyInOrg(orgId, input.companyId);
    ...
    if (input.templateId && !template)
        throw new QuoteValidationError('Không tìm thấy mẫu báo giá');
    ...
    const totals = calcTotals(input.lines.map((l) => ({ quantity: Number(l.quantity), unitPrice: Number(l.unitPrice), discountPercent: Number(l.discountPercent ?? 0) })), { discountType: input.discountType, discountValue: input.discountValue, taxRate });
    ...
    if (input.assignedUserId) {
        ...
        if (!assignee)
            throw new QuoteValidationError('Người phụ trách không thuộc tổ chức này');
    }
...
export async function updateQuote(orgId, id, user, input) {
    ...
    if (input.title !== undefined)
        data.title = input.title?.slice(0, 300) ?? null;
    if (input.notes !== undefined)
        data.notes = input.notes?.slice(0, 5000) ?? null;
    if (input.internalNotes !== undefined)
        data.internalNotes = input.internalNotes?.slice(0, 5000) ?? null;
    ...
    const updated = await prisma.$transaction(async (tx) => {
        if (touchesMoney) {
            ...
            const totals = calcTotals(rawLines.map((l) => ({ quantity: Number(l.quantity), unitPrice: Number(l.unitPrice), discountPercent: Number(l.discountPercent ?? 0) })), { discountType, discountValue, taxRate });
...
export async function softDeleteQuote(orgId, id, user) {
...
// ── Vòng đời ─────────────────────────────────────────────────────────
export async function markQuoteSent(orgId, id, user, sentVia) {
    const existing = await prisma.quote.findFirst({
        where: { id, orgId, deletedAt: null },
    ...
    if (existing.lines.length === 0)
        throw new QuoteValidationError('Báo giá chưa có dòng hàng nào');
 ...
 */
export async function duplicateQuote(orgId, id, user, targetContactId) {
...
export async function reviseQuote(orgId, id, user) {
    const source = await prisma.quote.findFirst({
        where: { id, orgId, deletedAt: null },
        select: { ...DETAIL_SELECT, lines: { select: LINE_SELECT, orderBy: { sortOrder: 'asc' } } },
    });
...
export async function convertToContract(orgId, id, user) {
    const source = await prisma.quote.findFirst({
        where: { id, orgId, deletedAt: null },
        select: { ...DETAIL_SELECT, lines: { select: LINE_SELECT, orderBy: { sortOrder: 'asc' } } },
    });
    ...
    if (source.status !== 'accepted')
        throw new QuoteStateError('Chỉ báo giá đã được chấp nhận mới chuyển thành hợp đồng');
    if (source.type === 'contract')
        throw new QuoteStateError('Đây đã là hợp đồng');
    const created = await prisma.$transaction(async (tx) => {
...
export async function respondByStaff(orgId, id, user, action, reason) {
    const existing = await prisma.quote.findFirst({ where: { id, orgId, deletedAt: null }, select: { id: true, status: true } });
    if (!existing)
        throw new QuoteNotFoundError();
