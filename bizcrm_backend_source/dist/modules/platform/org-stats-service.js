...
const EMPTY = { users: 0, contacts: 0, conversations: 0 };
/** Per-org counts for a given set of orgIds. Missing orgs default to zero. */
export async function getOrgStatsMap(orgIds) {
    const map = new Map();
    if (orgIds.length === 0)
        return map;
    ...
    for (const r of users)
        ensure(r.orgId).users = r._count._all;
    for (const r of contacts)
        ensure(r.orgId).contacts = r._count._all;
    for (const r of conversations)
        ensure(r.orgId).conversations = r._count._all;
    for (const id of orgIds)
        ensure(id); // guarantee an entry for every requested org
    return map;
}
...
//# sourceMappingURL=org-stats-service.js.map