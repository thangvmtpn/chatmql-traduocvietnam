 ...
 */
export async function executeNodeAction(actionType, config, ctx) {
    const params = config;
    switch (actionType) {
        case 'send_template': {
            const { templateId } = params;
            if (!ctx.conversationId || !templateId)
                break;
            const tmpl = await prisma.messageTemplate.findFirst({
                where: { id: templateId, orgId: ctx.orgId },
            });
            if (!tmpl) {
                logger.warn(`[automation] Template ${templateId} not found`);
                break;
            }
        ...
        case 'send_message': {
            const { text } = params;
            if (!ctx.conversationId || !text)
                break;
            await trySendViaZalo(ctx, text);
            return { action: 'send_message', text: text.slice(0, 100) };
        }
        case 'assign_agent': {
            const { userId } = params;
            if (!ctx.contactId || !userId)
                break;
        ...
        case 'change_status': {
            ...
            const targetStage = (lifecycleStage ?? status);
            if (!ctx.contactId || !targetStage)
                break;
            await prisma.contact.update({
                where: { id: ctx.contactId },
        ...
        case 'update_property': {
            ...
            if (!prop) {
                logger.warn(`[automation] Property ${fieldKey} not found`);
                break;
            }
        ...
        case 'send_zalo_zns': {
            const { accountId, templateId, templateData, phone: phoneOverride } = params;
        ...
        case 'ai_cdp': {
            ...
            if (!ctx.contactId)
                throw new Error('Thiếu contactId — chọn khách hàng để chạy AI-CDP');
            if (!ctx.conversationId)
                throw new Error('Contact chưa có hội thoại — AI-CDP cần ít nhất 1 cuộc trò chuyện');
 ...
 */
export async function trySendViaZalo(ctx, text) {
    if (!ctx.conversationId)
        return;
    try {
...
//# sourceMappingURL=automation-actions.js.map