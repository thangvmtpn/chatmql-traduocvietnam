...
import { ENGINE_CONFIG, buildAdjacency, getNextNodes, evaluateConditionNode, } from './automation-types.js';
 ...
 */
export async function executeFlowV2(ruleId, ruleName, flowConfig, ctx, dryRun = false) {
    ...
    let currentLevel = getNextNodes(adj, flowConfig.trigger.id);
    while (currentLevel.length > 0) {
        ...
        for (const nodeId of currentLevel) {
            if (visited.has(nodeId) || nodeId === 'node_end')
                continue;
            visited.add(nodeId);
            ...
            for (let ci = 0; ci < parallelBatch.length; ci += concurrency) {
                ...
                const results = await Promise.allSettled(chunk.map(async (node) => {
                    ...
                    if (node.type === 'note' || !node.actionType) {
                        return { nodeId: node.id, nodeLabel: node.label, status: 'skipped', durationMs: 0 };
                    }
                    try {
                        const output = await executeNodeAction(node.actionType, node.config, ctx);
                        return { nodeId: node.id, nodeLabel: node.label, status: 'success', durationMs: Date.now() - nodeStart, output };
                    }
                    catch (err) {
                        logger.error({ err }, `[automation] Node ${node.id} (${node.actionType}) failed`);
                        return { nodeId: node.id, nodeLabel: node.label, status: 'error', durationMs: Date.now() - nodeStart, error: err.message || String(err) };
                    }
                }));
        ...
        for (const nodeId of sequentialQueue) {
            ...
            if (node.type === 'condition') {
                ...
                if (unit === 'minutes')
                    delayMs = duration * 60_000;
                else if (unit === 'hours')
                    delayMs = duration * 3600_000;
                else if (unit === 'days')
                    delayMs = duration * 86400_000;
                if (dryRun) {
        ...
        data: {
            ...
            details: {
                ...
                context: {
                    ...
                    contactName: ctx['contact.fullName'] || null,
 ...
 */
export async function runAutomationRules(trigger, ctx, mode = 'queue') {
...
async function executeTriggerSync(trigger, ctx) {
    ...
    if (ctx.contactId && !ctx['contact.id']) {
    ...
    for (const rule of rules) {
        const flowConfig = rule.flowConfig;
        if (flowConfig && flowConfig.trigger && Array.isArray(flowConfig.nodes)) {
            ...
            if (trigger === 'conversation_idle') {
                const ruleThreshold = flowConfig.trigger.config?.threshold || '30m'; // default matches UI
                const ctxThreshold = ctx.triggerData?.idleThreshold;
                if (ctxThreshold && ruleThreshold !== ctxThreshold)
                    continue;
            }
            try {
                const result = await executeFlowV2(rule.id, rule.name, flowConfig, ctx);
                matched++;
 ...
 */
export async function processTriggerJob(trigger, ctx) {
    return executeTriggerSync(trigger, ctx);
}
/**
...
export async function processDelayJob(data) {
    const flowConfig = data.flowConfig;
            ...
            case 'delay': {
                ...
                if (ms <= ENGINE_CONFIG.MAX_INLINE_DELAY_MS) {
                    ...
                    await enqueueDelay({
                        ...
                        flowConfig: flowConfig, resumeFromNodeIds: nextIds,
                        context: ctx, scheduledFor: new Date(Date.now() + ms).toISOString(),
                    }, ms);
                    nodeResults.push({ nodeId: node.id, nodeLabel: node.label, status: 'success', durationMs: Date.now() - nodeStart });
                }
                break;
...
export async function pollDelayJobs() {
    const now = new Date();
    ...
    for (const job of jobs) {
        try {
            ...
            const flowConfig = job.flowConfig;
            const ctx = job.context;
            const resumeNodeIds = job.resumeFromNodeIds;
                    ...
                    case 'delay': {
                        ...
                        if (ms <= 30_000) {
                            ...
                            await prisma.automationDelayJob.create({
                                data: { orgId: ctx.orgId, ruleId: job.ruleId, flowConfig: flowConfig, resumeFromNodeIds: nextIds, context: ctx, executeAt: new Date(Date.now() + ms), status: 'pending' },
                            });
                            nodeResults.push({ nodeId: node.id, nodeLabel: node.label, status: 'success', durationMs: Date.now() - nodeStart });
                        }
                        break;
 ...
 */
export async function pollTriggerJobs() {
