...
function handleEvent(e) {
    if (e.origin === PERFEX_ORIGIN)
        return; // loop guard: ignore our own (future) reverse-sync writes
    if (!isPerfexEnabled(e.orgId))
        return; // optional / per-org — no work when disabled
...
export function registerPerfexIntegration() {
    registerIntegration({
        name: 'perfex',
        init: async () => {
...
//# sourceMappingURL=perfex-subscriber.js.map