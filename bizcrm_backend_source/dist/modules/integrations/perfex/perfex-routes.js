...
const isAdmin = (u) => ['owner', 'admin'].includes(u.role);
...
export async function perfexRoutes(app) {
    ...
    app.put('/api/v1/integrations/perfex/config', async (request, reply) => {
        const user = request.user;
        if (!isAdmin(user))
            return reply.status(403).send({ error: 'Only admin/owner can manage integrations' });
        ...
        if (existing) {
            await prisma.integration.update({
                ...
                data: { enabled, config: merged },
            });
        }
        else {
            await prisma.integration.create({
                data: { orgId: user.orgId, type: PERFEX_INTEGRATION_TYPE, name: 'PerfexCRM', enabled, config: merged },
            });
        }
    ...
    app.post('/api/v1/integrations/perfex/test', { config: { rateLimit: { max: 5, timeWindow: '1 minute' } } }, // throttle SSRF/probing
    async (request, reply) => {
        const user = request.user;
        if (!isAdmin(user))
            return reply.status(403).send({ error: 'Only admin/owner can manage integrations' });
        try {
            ...
            if (!baseUrl || !authToken)
                return { ok: false, error: 'Thiếu baseUrl hoặc authToken' };
    ...
    app.post('/api/v1/integrations/perfex/sync-now', { config: { rateLimit: { max: 3, timeWindow: '1 minute' } } }, async (request, reply) => {
        ...
        if (!integration?.enabled)
            return reply.status(400).send({ error: 'PerfexCRM chưa được bật' });
        ...
        const rows = [
            ...companies.map((c) => ({ orgId: user.orgId, localType: 'company', localId: c.id, op: 'upsert', status: 'pending' })),
            ...contacts.map((c) => ({ orgId: user.orgId, localType: 'contact', localId: c.id, op: 'upsert', status: 'pending' })),
        ];
        if (rows.length > 0) {
    ...
    app.get('/api/v1/integrations/perfex/logs', async (request, reply) => {
        const user = request.user;
        if (!isAdmin(user))
            return reply.status(403).send({ error: 'Only admin/owner can manage integrations' });
...
//# sourceMappingURL=perfex-routes.js.map