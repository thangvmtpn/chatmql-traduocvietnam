...
function makeClient() {
    ...
    return {
        ...
        searchContacts: vi.fn().mockImplementation((q) => Promise.resolve(searchCalls++ === 0 ? [] : [{ id: '55', email: q }])),
...
const ctx = (client) => ({
    orgId: 'o1', integrationId: 'int1', config: config(), client: client,
});
const contact = (over = {}) => ({
...
describe('pushCompany', () => {
    const company = { id: 'co1', name: 'Acme', taxCode: null, phone: null, website: null, address: null };
    ...
    it('skips the API call when payload hash is unchanged', async () => {
        const client = makeClient();
        const hash = payloadHash(companyToCustomer(company));
...
//# sourceMappingURL=perfex-push.test.js.map