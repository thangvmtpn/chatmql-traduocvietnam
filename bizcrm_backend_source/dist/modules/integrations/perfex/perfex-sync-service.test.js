...
const row = (over) => ({ id: 'r', orgId: ORG, localType: 'contact', localId: 'c1', op: 'upsert', status: 'pending', createdAt: new Date(), ...over });
beforeEach(() => {
    vi.clearAllMocks();
...
describe('flushOrg', () => {
    it('no pending rows → no work', async () => {
        ;
        loadPerfexIntegration.mockResolvedValue(integration());
    ...
    it('integration disabled → mark rows skipped, no push', async () => {
        ;
        loadPerfexIntegration.mockResolvedValue({ ...integration(), enabled: false });
    ...
    it('routes lead-stage contact to pushLead when syncLeads on', async () => {
        ;
        loadPerfexIntegration.mockResolvedValue(integration({ syncLeads: true }));
...
//# sourceMappingURL=perfex-sync-service.test.js.map