...
export async function getFmPool() {
    if (!fmPool) {
        try {
            const mod = 'pg';
            const pgPkg = await import(/* @vite-ignore */ mod);
...
export async function ensureCustomProperties(orgId) {
    const defaultProps = [
...
export async function findCrmCustomerByPhone(phone) {
    ...
    if (phones.length === 0)
        return null;
    try {
        const res = await pool.query(`SELECT id_kh, ten_khach_hang, sdt1, sdt2, gioi_tinh, ngay_sinh, nguon_data, nhom_kh, 
...
export async function syncContactFromCrm(contactId, orgId) {
    ...
    const currentMeta = (contact.metadata && typeof contact.metadata === 'object') ? { ...contact.metadata } : {};
    if (crmKh.dia_chi)
        currentMeta.address = crmKh.dia_chi.trim();
    if (crmKh.tinh)
        currentMeta.city = crmKh.tinh.trim();
...
export async function batchSyncContacts(orgId, limit = 500) {
...
//# sourceMappingURL=crm-sync-service.js.map