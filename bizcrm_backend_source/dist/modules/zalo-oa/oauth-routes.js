...
import { buildAuthorizeUrl, exchangeCodeForToken, OaClient, OaApiError, } from './oa-client.js';
...
export async function zaloOaOAuthRoutes(app) {
    // ─── authed: start ─────────────────────────────────────────────────────────
    app.get('/api/v1/zalo-oa/connect/start', { preHandler: authMiddleware }, async (request, reply) => {
        ...
        await redisConnection.set(`oa:oauth:state:${state}`, JSON.stringify({ orgId: user.orgId, userId: user.id, verifier }), 'EX', STATE_TTL_SEC);
    ...
    app.get('/api/v1/zalo-oa/callback', async (request, reply) => {
        ...
        try {
            ...
            const oa = await client.getOaInfo().catch((err) => {
                ...
                return { externalPageId: '', name: 'Zalo OA', avatar: undefined };
            });
