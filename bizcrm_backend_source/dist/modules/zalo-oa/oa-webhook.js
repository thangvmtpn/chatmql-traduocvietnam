...
function detectContentType(message) {
    if (!message)
        return { contentType: 'text', content: '', attachments: [] };
    if (message.attachments && message.attachments.length > 0) {
        const t = message.attachments[0].type;
        if (t === 'image' || t === 'photo')
            return { contentType: 'image', content: message.text ?? '', attachments: message.attachments };
        if (t === 'file' || t === 'audio' || t === 'video')
            return { contentType: t, content: message.text ?? '', attachments: message.attachments };
        if (t === 'gif')
            return { contentType: 'gif', content: '', attachments: message.attachments };
        if (t === 'sticker')
            return { contentType: 'sticker', content: '', attachments: message.attachments };
        if (t === 'location')
            return { contentType: 'location', content: message.text ?? '📍 Vị trí', attachments: message.attachments };
    }
    if (message.sticker?.sticker_id)
        return { contentType: 'sticker', content: '', attachments: [message.sticker] };
    if (message.link)
        return { contentType: 'link', content: message.link, attachments: [] };
    return { contentType: 'text', content: message.text ?? '', attachments: [] };
}
export async function zaloOaWebhookRoutes(app) {
    app.post('/api/v1/zalo-oa/webhook', async (request, reply) => {
        const body = request.body;
        const strict = process.env.OA_WEBHOOK_STRICT === 'true';
        const sigHeader = (request.headers['x-zevent-signature'] ?? request.headers['x-zalo-signature']);
        ...
        const oaCandidates = [body.oa_id, body.recipient?.id, body.sender?.id].filter(Boolean);
        if (oaCandidates.length === 0)
            return reply.send({ ok: true, ignored: 'no_oa_id' });
        let account = null;
...
async function routeEvent(body, accountId, orgId, externalPageId) {
    ...
    if (eventName.startsWith('oa_send_')) {
        ...
        if (externalMsgId) {
            ...
            if (conv) {
                ...
                if (existing)
                    return; // Already tracked (sent from CRM)
            }
        }
        const { contentType, content, attachments } = detectContentType(body.message);
...
async function enrichOaContact(accountId, senderUid, orgId) {
    try {
        const client = await getClient(accountId);
        const profile = await client.getFollowerProfile(senderUid);
        if (!profile.displayName && !profile.avatar && !profile.phone)
            return;
        const contact = await prisma.contact.findFirst({
        ...
        if (profile.displayName) {
            ...
            if (isAutoName && profile.displayName !== current) {
                ...
                }).catch((err) => logger.warn(`[oa-webhook] displayName sync failed: ${err.message}`));
            }
        }
        ...
        if (Object.keys(updates).length > 0) {
            ...
            if (updates.avatarUrl) {
                tryAutoMergeOaByAvatar(contact.id, updates.avatarUrl, orgId)
...
async function tryAutoMergeOaByAvatar(oaContactId, avatarUrl, orgId) {
    ...
    if (!oaContact || oaContact.mergedInto || oaContact.deletedAt)
        return;
    ...
    const allContacts = [oaContact, ...matches].sort((a, b) => new Date(a.createdAt).getTime() - new Date(b.createdAt).getTime());
...
async function applyZnsStatus(trackingId, status) {
    if (!trackingId)
        return;
    try {
...
async function insertSystemMessage(accountId, orgId, userUid, contactId, text, timestamp) {
    try {
...
//# sourceMappingURL=oa-webhook.js.map