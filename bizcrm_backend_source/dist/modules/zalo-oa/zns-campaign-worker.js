...
import { enqueueZnsSend } from '../../shared/queue.js';
...
export async function processZnsSendJob(job) {
...
async function markRecipient(recipientId, status, extra = {}) {
    await prisma.znsCampaignRecipient.update({
        where: { id: recipientId },
        data: { status, processedAt: new Date(), ...extra },
    });
}
async function bumpCampaign(campaignId, delta) {
...
export async function startCampaign(campaignId) {
    ...
    const ids = Array.isArray(campaign.contactIds) ? campaign.contactIds : [];
    if (ids.length === 0)
...
//# sourceMappingURL=zns-campaign-worker.js.map