...
export async function analyticsRoutes(app) {
    ...
    const FUNNEL_STAGES = ['subscriber', 'lead', 'qualified', 'opportunity', 'customer'];
    const STAGE_WEIGHT = {
    ...
    app.get('/api/v1/analytics/conversion-funnel', async (request) => {
        const user = request.user;
    ...
    app.get('/api/v1/analytics/team-performance', async (request) => {
        const user = request.user;
        const { from, to } = request.query;
        const fromDate = from ? new Date(from) : new Date(Date.now() - 30 * 86400_000);
        const toDate = to ? new Date(to) : new Date();
    ...
    app.get('/api/v1/analytics/response-time', async (request) => {
        const user = request.user;
        const { from, to } = request.query;
        const fromDate = from ? new Date(from) : new Date(Date.now() - 30 * 86400_000);
        const toDate = to ? new Date(to) : new Date();
    ...
    app.get('/api/v1/analytics/kpi-summary', async (request) => {
        const user = request.user;
        const { from, to } = request.query;
        const fromDate = from ? new Date(from) : new Date(Date.now() - 30 * 86400_000);
        const toDate = to ? new Date(to) : new Date();
    ...
    app.post('/api/v1/saved-reports/:id/run', async (request, reply) => {
        ...
        switch (report.type) {
            ...
            case 'conversion-funnel': {
                ...
                const counts = {};
                for (const g of groups)
                    counts[g.lifecycleStage ?? 'unknown'] = g._count.id;
