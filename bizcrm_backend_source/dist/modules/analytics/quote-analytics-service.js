...
export async function getQuoteAnalytics(orgId, from, to) {
    ...
    const [statusGroups, closedRows, userGroups, users, lineRows, reasonRows, trendRows, pipelineAgg] = await Promise.all([
        // Đếm theo trạng thái
    ...
    for (const g of userGroups) {
        ...
        if (g.status !== 'draft' && g.status !== 'canceled')
            row.sent += n;
        if (g.status === 'accepted') {
            row.accepted += n;
            row.wonValue += Number(g._sum.total ?? 0);
        }
    ...
    return {
        ...
        funnel,
        trend: fillPeriods(trendRows.map((r) => ({ period: r.period, count: Number(r.count), value: Number(r.value) })), from, to),
...
//# sourceMappingURL=quote-analytics-service.js.map