...
async function phonesWithInvoices(phones) {
    const url = process.env.CRM_DATABASE_URL;
    if (!url || !phones.length)
        return new Set();
    try {
        ...
        try {
            ...
            const res = await client.query(`SELECT DISTINCT regexp_replace(sdt, '\\D', '', 'g') AS p FROM hoa_don
         WHERE regexp_replace(sdt, '\\D', '', 'g') = ANY($1)`, [norm]);
            return new Set(res.rows.map((r) => r.p));
        }
        finally {
            await client.end();
        }
        ...
        return new Set();
    } // CRM không với tới được → rơi về heuristic
}
const STATUS_LABELS = {
    won: 'Chốt thành công',
...
async function buildTranscriptFromSystem(orgId, opts) {
    ...
    for (const c of convs) {
        ...
        if (staffTurns.length < 3 || custTurns.length < 2)
            continue;
        const timeline = [
            ...msgs.map((m) => ({
                type: 'msg',
        ...
        if (!lines.some((l) => l.startsWith('NV: ')))
            continue;
        let score = staffTurns.length;
        if (opts.preferOrders && lines.some((l) => CLOSE_KEYWORDS.test(l)))
            score += 50;
        // Ưu tiên hội thoại có ghi chú chốt đơn hoặc ghi chú chăm sóc chi tiết
        if (notes.some((n) => n.status === 'won' || CLOSE_KEYWORDS.test(n.content)))
            score += 80;
        else if (notes.length > 0)
            score += 25;
        if (c.contact?.phone)
            phoneByConv.set(c.id, c.contact.phone);
    ...
    for (const [i, s] of picked.entries()) {
        const block = `\n===== HỘI THOẠI ${i + 1} =====\n` + s.lines.join('\n') + '\n';
        if (transcript.length + block.length > MAX_TRANSCRIPT_CHARS)
            break;
        transcript += block;
        used++;
    }
...
// ── Gọi model ───────────────────────────────────────────────────────────────
async function dispatchProvider(provider, apiKey, model, system, userPrompt, maxTokens = 3000) {
...
async function analyzeAndPropose(orgId, transcript, sampled, sourceLabel, minChars = 500, tooShortMsg = 'Không đủ hội thoại đạt chuẩn để học (cần các cuộc có ≥3 lượt nhân viên trả lời). Nới khoảng thời gian hoặc chọn kênh khác.') {
    ...
    const userPrompt = `PERSONA HIỆN TẠI:\n-----\n${currentPersona}\n-----\n\n` +
    ...
    if (!parsed)
        throw new Error('Model không trả về đề xuất hợp lệ — thử lại giúp em');
...
export default async function learnHistoryRoutes(app) {
    ...
    app.post('/api/v1/ai/learn-history/analyze', async (request, reply) => {
        const user = request.user;
        if (!isAdmin(user.role))
            return sendError(reply, 403, 'Chỉ admin/owner được chạy học lịch sử');
    ...
    app.post('/api/v1/ai/learn-history/analyze-file', async (request, reply) => {
        ...
        try {
            ...
            if (!file)
                return sendError(reply, 400, 'Chưa đính kèm tệp');
            const buf = await file.toBuffer();
            let text = buf.toString('utf8');
            // JSON export → ép về text phẳng để phân tích
            if (file.filename.endsWith('.json')) {
                try {
                    text = JSON.stringify(JSON.parse(text), null, 1).replace(/[{}\[\]",]/g, ' ');
                }
                catch { /* giữ nguyên */ }
            }
            const transcript = anonymize(text).slice(0, MAX_TRANSCRIPT_CHARS);
            const out = await analyzeAndPropose(user.orgId, transcript, 1, 'learn_history_file', 200, 'Tệp quá ngắn để học được gì — cần ít nhất vài lượt hội thoại của nhân viên.');
...
//# sourceMappingURL=learn-history-routes.js.map