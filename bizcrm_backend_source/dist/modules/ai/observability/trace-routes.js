...
export async function traceRoutes(app) {
    app.addHook('preHandler', authMiddleware);
    app.get('/api/v1/ai/trace', async (request, reply) => {
     ...
     */
    app.get('/api/v1/ai/reply-runs', async (request, reply) => {
        if (!ownerAdminOnly(request, reply))
            return;
        const user = request.user;
