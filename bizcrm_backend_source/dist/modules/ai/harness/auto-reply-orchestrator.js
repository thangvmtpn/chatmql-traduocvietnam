...
export async function processAiReply(convId) {
    ...
    const timer = setTimeout(() => ac.abort(new Error(`AI turn exceeded ${TURN_BUDGET_MS}ms budget`)), TURN_BUDGET_MS);
    try {
...
async function advanceCursor(convId, messageId) {
    await prisma.conversation.update({
        where: { id: convId },
        data: { lastAiProcessedMessageId: messageId },
    });
}
async function executeAiReplyPipeline(convId, signal) {
    // ── 1. Nạp hội thoại ─────────────────────────────────────────────────────
    ...
    if (effectiveMode === 'auto' && conv.channelAccount?.platform === Platform.ZALO_USER) {
        ...
        if (recipient) {
            sendTypingViaPool(conv.channelAccountId, recipient, conv.threadType).catch(() => { });
        }
    }
    let failure = null;
    let result = null;
 ...
 */
export async function recoverPendingAiReplies() {
  ...
  `;
    ...
    if (n)
        logger.info({ count: n }, '[orchestrator] re-scheduled AI replies pending from before restart');
    return n;
}
