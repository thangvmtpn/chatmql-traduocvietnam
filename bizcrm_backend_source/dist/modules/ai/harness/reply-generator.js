...
import { resolveBotForConversation, resolveBotById } from '../ai-bot-service.js';
...
opts = {}) {
    ...
    const ctx = await assembleContext(orgId, convId, turnText, provisionalRunId, {
        skipRag: agentMode,
    ...
    if (bot?.personaPrompt)
        ctx.logic.persona = truncate(bot.personaPrompt, budgets.persona);
    if (bot?.playbookPrompt)
        ctx.logic.playbook = truncate(bot.playbookPrompt, budgets.playbook);
    // ── Pass 1: router ──────────────────────────────────────────────────────────
