...
import { TOOL_NAMES } from '../tools-config-service.js';
...
const SEND_IMAGE_DEF = {
    type: 'function',
    function: {
        name: SEND_IMAGE_TOOL,
        description: 'Gửi ẢNH THẬT của một sản phẩm cho khách qua Zalo. Dùng khi khách hỏi xem ảnh, ' +
...
const ORDER_DEF = {
    ...
    function: {
        name: ORDER_TOOL,
        description: 'Lên ĐƠN HÀNG NHÁP / Ghi nhận đơn mua hàng khi khách đồng ý chốt đơn hoặc đã cung cấp thông tin nhận hàng (Tên, SĐT, Địa chỉ, Sản phẩm). ' +
...
function formatProducts(rows) {
    if (rows.length === 0)
        return 'Không tìm thấy sản phẩm phù hợp trong phạm vi cho phép.';
    return rows.map((p) => {
...
function formatKb(rows) {
    if (rows.length === 0)
        return 'Không tìm thấy thông tin phù hợp trong phạm vi cho phép.';
...
function scopeRows(rows, allowed) {
    return rows
...
async function formatOverview(orgId, tools) {
    ...
    if (tools.search_knowledge.enabled) {
        ...
        if (total > 0)
            lines.push(`KIẾN THỨC/FAQ (tổng ${total}):\n${kb.map((k) => `• ${k.category}: ${k.count}`).join('\n')}`);
    }
    if (lines.length === 0)
        return 'Chưa có dữ liệu danh mục trong phạm vi được cấp.';
 ...
 */
export async function resolveProductImage(orgId, query) {
    const q = (query || '').trim();
    if (!q)
...
export async function executeTool(orgId, name, args, tools, topK, minScore) {
    if (name === CATALOG_OVERVIEW_TOOL)
        return { text: await formatOverview(orgId, tools), hits: [] };
    if (!isToolName(name))
        return { text: `Công cụ không tồn tại: ${name}`, hits: [] };
    const tool = tools[name];
    if (!tool.enabled)
        return { text: 'Công cụ này hiện đang tắt.', hits: [] };
    const query = typeof args?.query === 'string' ? args.query.trim() : '';
    if (!query)
        return { text: 'Thiếu tham số "query".', hits: [] };
 ...
 * carry it.
 */
function notFoundMsg(guardrailIds, kind) {
