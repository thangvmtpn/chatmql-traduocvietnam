...
export const TOOL_NAMES = ['search_products', 'search_knowledge'];
...
export function parseToolsDoc(content) {
    ...
    try {
        ...
        for (const name of TOOL_NAMES) {
            if (tools && tools[name] !== undefined)
                out[name] = coerceTool(tools[name]);
        }
    }
    catch {
...
export function serializeToolsDoc(cfg) {
 ...
 */
export async function applyToolsPatch(orgId, patch, updatedBy) {
    ...
    const toolsPatch = (patch.tools ?? patch);
    for (const name of TOOL_NAMES) {
    ...
    if (!touched)
        throw new Error('Không có trường tool/guardrail hợp lệ để cập nhật');
    ...
    if (prodIds.length) {
        ...
        if (missing.length)
            throw new Error(`Danh mục sản phẩm không hợp lệ: ${missing.join(', ')}`);
    }
    if (knowIds.length) {
        ...
        if (missing.length)
            throw new Error(`Nhóm kiến thức không hợp lệ: ${missing.join(', ')}`);
    }
 ...
 */
export async function buildToolScopeNote(orgId, tools) {
    ...
    const renderScope = (cfg, label, cats) => {
        if (!cfg.enabled)
            return `- ${label}: ĐÃ TẮT — bạn KHÔNG có khả năng tra cứu phần này.`;
        const names = cfg.guardrail.categoryIds
            .map((id) => cats.find((c) => c.id === id)?.name)
            .filter(Boolean);
        if (names.length)
            return `- ${label}: CHỈ được tra trong danh mục: ${names.join(', ')}.`;
...
//# sourceMappingURL=tools-config-service.js.map