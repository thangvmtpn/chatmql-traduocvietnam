...
export async function createScenario(orgId, input, by) {
    ...
    await embedAndStoreScenario(orgId, row.id).catch(() => { });
    return row;
}
export async function updateScenario(orgId, id, patch, by) {
    ...
    if (nextName !== cur.name || nextDesc !== cur.description || nextHints !== cur.triggerHints) {
        await embedAndStoreScenario(orgId, id).catch(() => { });
    }
    return row;
}
export async function deleteScenario(orgId, id) {
    const res = await prisma.aiScenario.deleteMany({ where: { id, orgId } });
    return res.count > 0;
}
export async function getScenarioVersions(orgId, id) {
...
// ── Embedding ───────────────────────────────────────────────────────────────
export async function embedAndStoreScenario(orgId, id) {
    ...
    await prisma.$executeRaw(Prisma.sql `UPDATE ai_scenarios SET embedding = ${literal}::vector WHERE id = ${id} AND org_id = ${orgId}`);
    return true;
}
/** Idempotent backfill of scenario embeddings (enabled, missing a vector). */
export async function backfillScenarioEmbeddings(orgId) {
    const rows = await prisma.$queryRaw(Prisma.sql `SELECT id FROM ai_scenarios WHERE org_id = ${orgId} AND enabled = true AND embedding IS NULL`);
    let embedded = 0, failed = 0;
    for (const r of rows) {
        (await embedAndStoreScenario(orgId, r.id)) ? embedded++ : failed++;
    }
...
export async function getAlwaysScenarios(orgId) {
    const rows = await prisma.aiScenario.findMany({
        where: { orgId, enabled: true, loadMode: 'always' },
        orderBy: [{ priority: 'desc' }, { updatedAt: 'desc' }],
        select: { id: true, name: true, content: true },
    });
    return rows.map((r) => ({ id: r.id, name: r.name, content: r.content, loadMode: 'always', score: null }));
}
/**
 ...
 */
export async function retrieveRelevantScenarios(orgId, query, topK, minScore = DEFAULT_RAG_MIN_SCORE) {
    ...
    try {
      ...
      `);
        return rows
            ...
            .map((r) => ({
            id: r.id, name: r.name, content: r.content, loadMode: 'auto',
