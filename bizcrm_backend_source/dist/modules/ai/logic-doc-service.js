...
const VALID_DOC_TYPES = ['index', 'persona', 'playbook', 'handoff_rules', 'mechanism', 'criteria', 'tools'];
export function isValidDocType(t) {
    return VALID_DOC_TYPES.includes(t);
}
// ── Queries ────────────────────────────────────────────────────────
/**
 * List all logic docs for an org (meta only, no content — for list view).
 */
export async function getLogicDocs(orgId) {
    ...
    return docs.map((d) => ({ ...d, content: undefined, contentLength: d.content.length }));
}
/**
 * Get a single logic doc by type (including full content).
 */
export async function getLogicDoc(orgId, type) {
 ...
 */
export async function getActiveLogicContext(orgId) {
    const docs = await prisma.aiLogicDoc.findMany({
 ...
 */
export async function upsertLogicDoc(orgId, type, content, updatedBy, changeNote) {
 ...
 */
export async function revertLogicDoc(orgId, type, targetVersion, revertedBy) {
    const doc = await prisma.aiLogicDoc.findFirst({ where: { orgId, type } });
    if (!doc)
        throw new Error(`Logic doc type "${type}" not found for this org`);
    ...
    if (!versionRow)
        throw new Error(`Version ${targetVersion} not found for doc type "${type}"`);
 ...
 */
export async function getLogicDocVersions(orgId, type) {
 ...
 */
export async function seedDefaultLogicDocs(orgId, createdBy = 'system') {
    ...
    for (const type of VALID_DOC_TYPES) {
        if (type === 'tools')
            continue; // config doc, not seeded as prose
...
//# sourceMappingURL=logic-doc-service.js.map