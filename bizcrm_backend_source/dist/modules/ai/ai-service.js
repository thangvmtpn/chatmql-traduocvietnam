...
import { buildReplyDraftPrompt, parseReplyDraftSuggestions } from './prompts/reply-draft.js';
...
// ── Provider dispatch ───────────────────────────────────────────────────
export async function dispatchProvider(provider, apiKey, model, system, userPrompt, options = {}) {
    ...
    if (provider === 'gemini')
        return generateWithGemini(baseUrl, apiKey, model, system, userPrompt, options);
...
export async function generateAiOutput(input) {
    ...
    if (input.type === 'reply_draft') {
        ...
        const saved = await Promise.all(items.map(item => prisma.aiSuggestion.create({
            data: {
                orgId: input.orgId,
                conversationId: input.conversationId,
        ...
        const suggestions = items.map((item, idx) => ({ suggestionId: saved[idx].id, tone: item.tone, text: item.text }));
        return { suggestions };
    }
    // summary branch
 ...
 */
export async function analyzeConversation(input, options = {}) {
    const [cfg, conv] = await Promise.all([
        getAiConfig(input.orgId),
        loadConversation(input.conversationId, input.orgId),
    ]);
...
export async function scoreLeadFromConversation(input) {
    ...
    const userPrompt = buildContextBlock(conv.messages, customerName);
    const language = detectLanguage(userPrompt);
    const raw = await dispatchProvider(provider, apiKey, model, buildLeadScoringPrompt(language), userPrompt, { jsonMode: true, maxTokens: 800 });
    await logUsage({
        orgId: input.orgId,
        provider,
        model,
