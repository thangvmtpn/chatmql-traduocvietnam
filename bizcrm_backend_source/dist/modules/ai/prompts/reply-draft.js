 ...
 */
export function parseReplyDraftSuggestions(raw) {
    let parsed;
    try {
    ...
    if (!root || typeof root !== 'object' || !Array.isArray(root.suggestions))
        return null;
    const validTones = ['concise', 'friendly', 'detailed'];
    const out = [];
    ...
    for (const item of root.suggestions) {
        if (!item || typeof item !== 'object')
            continue;
        const obj = item;
        const tone = typeof obj.tone === 'string' && validTones.includes(obj.tone)
            ? obj.tone
            : null;
