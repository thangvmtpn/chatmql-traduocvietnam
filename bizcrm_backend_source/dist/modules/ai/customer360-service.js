...
function buildPortrait(contact, crm, conv) {
    ...
    if (crm) {
        ...
        if (crm.priority_level)
            out.push(`Nhóm khách hàng: ${crm.priority_level}.`);
        if (crm.staff_in_charge)
            out.push(`Nhân sự phụ trách: ${crm.staff_in_charge}.`);
        if (crm.thich_dung_hang)
            out.push(`Gu dùng: ${crm.thich_dung_hang}.`);
        if (crm.nhu_cau_sd)
            out.push(`Nhu cầu: ${crm.nhu_cau_sd}.`);
    }
    else {
    ...
    if (bd) {
        ...
        if (isToday)
            out.push(`🎂 HÔM NAY là sinh nhật khách (${label}).`);
        else if (isMonth)
            out.push(`🎂 Sinh nhật khách trong tháng này (${label}).`);
    }
    if (crm?.next_sales_at || crm?.next_care_at) {
        const bits = [];
        if (crm.next_sales_at)
            bits.push(`bán hàng ${new Date(crm.next_sales_at).toLocaleDateString('vi-VN')}`);
        if (crm.next_care_at)
            bits.push(`chăm sóc ${new Date(crm.next_care_at).toLocaleDateString('vi-VN')}`);
...
async function askAi(orgId, conversationId, contextExtra) {
    const cfg = await getAiConfig(orgId);
    if (!cfg.enabled)
        throw new Error('AI đang tắt cho tổ chức này');
    ...
    const ctx = buildContextBlock([...conv.messages].reverse(), name);
    const userPrompt = `${ctx}\n\n<customer_profile>\n${contextExtra}\n</customer_profile>`;
    const raw = await dispatchProvider(provider, apiKey, cfg.model, SYSTEM_PROMPT, userPrompt, { jsonMode: true, maxTokens: 900 });
    return raw.text;
}
function parseAi(text) {
    try {
        ...
        return {
            ...
            actions: Array.isArray(j.actions) ? j.actions.map((a) => String(a).trim()).filter(Boolean) : [],
        };
    }
    catch {
...
export async function analyzeCustomer360(input) {
    ...
    if (cacheFresh) {
        const parsed = parseAi(contact.aiSummary);
        if (parsed.summary) {
            return {
                portrait,
...
//# sourceMappingURL=customer360-service.js.map