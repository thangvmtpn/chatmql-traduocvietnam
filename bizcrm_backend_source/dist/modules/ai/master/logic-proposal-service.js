 ...
 */
export async function listProposals(orgId, status, limit = 50, offset = 0) {
...
export async function createProposal(input) {
    ...
    if (!proposedValue)
        throw new Error('proposedValue cannot be empty');
 ...
 */
export async function rejectProposal(proposalId, orgId, reviewedBy) {
    ...
    if (proposal.status === 'applied')
        throw new Error('Cannot reject an already-applied proposal');
    ...
    }).catch((err) => logger.error({ err, proposalId }, '[master] activityLog write failed'));
    return { proposalId };
}
/**
 ...
 */
export async function applyProposal(proposalId, orgId, reviewedBy) {
    const proposal = await prisma.aiLogicProposal.findFirst({
        where: { id: proposalId, orgId },
    });
    if (!proposal)
        throw new Error(`Proposal ${proposalId} not found`);
    if (proposal.status === 'applied')
        throw new Error('Proposal already applied');
    if (proposal.status === 'rejected')
    ...
    if (proposal.targetType === 'logic_doc') {
        ...
        if (!subtype || !isValidDocType(subtype)) {
            throw new Error(`logic_doc proposal requires a valid targetSubtype (index|persona|playbook|handoff_rules|mechanism|criteria). Got: ${subtype}`);
        }
        const updated = await upsertLogicDoc(orgId, subtype, proposal.proposedValue, 'ai_master', `AI Master proposal ${proposalId}: ${proposal.rationale.slice(0, 120)}`);
        ...
        if (!proposal.targetId)
            throw new Error('knowledge_entry delete proposal requires targetId');
        const deleted = await deleteKbEntry(orgId, proposal.targetId);
        if (!deleted)
            throw new Error('Knowledge entry not found (already deleted?)');
        ...
        if (proposal.targetId) {
            ...
            const updated = await updateKbEntry(orgId, proposal.targetId, { content: proposal.proposedValue, risk, changeNote: `AI Master proposal ${proposalId}` }, 'ai_master');
        ...
        if (proposal.targetSubtype === 'delete') {
            if (!proposal.targetId)
                throw new Error('product delete proposal requires targetId');
            ...
            let patch;
            try {
                patch = JSON.parse(proposal.proposedValue);
            ...
            const updated = await updateProduct(orgId, proposal.targetId, patch);
            if (!updated)
            ...
            const p = await createProduct(orgId, { ...input, source: 'ai_master' }, undefined);
            appliedRef = p.id;
        }
        ...
        if (proposal.targetSubtype === 'delete') {
            if (!proposal.targetId)
                throw new Error('scenario delete proposal requires targetId');
            const ok = await deleteScenario(orgId, proposal.targetId);
            if (!ok)
                throw new Error('Scenario not found (already deleted?)');
            appliedRef = proposal.targetId;
        }
        else {
            let data;
            try {
                data = JSON.parse(proposal.proposedValue);
            ...
            if (proposal.targetId) {
                const updated = await updateScenario(orgId, proposal.targetId, { ...data, changeNote: `AI Master proposal ${proposalId}` }, 'ai_master');
                if (!updated)
                ...
                const created = await createScenario(orgId, data, 'ai_master');
        ...
        if (patch.ragMinScore !== undefined) {
            ...
            if (!Number.isFinite(v) || v < 0 || v > 1)
                throw new Error('ragMinScore must be between 0 and 1');
            data.ragMinScore = v;
        }
        if (patch.ragTopK !== undefined) {
            const v = Number(patch.ragTopK);
            if (!Number.isInteger(v) || v < 1 || v > 50)
                throw new Error('ragTopK must be an integer between 1 and 50');
            data.ragTopK = v;
        }
        if (Object.keys(data).length === 0)
            throw new Error('retrieval_tuning: nothing to change (set ragMinScore and/or ragTopK)');
    ...
    if (proposal.feedbackId) {
        await updateFeedbackStatus(proposal.feedbackId, orgId, 'applied').catch((err) => logger.error({ err, proposalId }, '[master] feedback status update failed'));
    }
...
async function applyGuardrailPatch(orgId, proposedValue) {
    let patch;
    try {
        patch = JSON.parse(proposedValue);
    }
    catch {
...
//# sourceMappingURL=logic-proposal-service.js.map