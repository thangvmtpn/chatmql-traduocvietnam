...
export async function aiRoutes(app) {
    ...
    app.put('/api/v1/ai/config', async (request, reply) => {
        try {
            const user = request.user;
    ...
    app.post('/api/v1/ai/config/apply-default-mode', async (request, reply) => {
        try {
            const user = request.user;
            if (!['owner', 'admin'].includes(user.role)) {
    ...
    app.put('/api/v1/ai/schedule', async (request, reply) => {
        try {
            const user = request.user;
            if (!['owner', 'admin'].includes(user.role)) {
    ...
    app.put('/api/v1/ai/tools-config', async (request, reply) => {
        try {
            const user = request.user;
            if (!['owner', 'admin'].includes(user.role)) {
    ...
    app.get('/api/v1/ai/pending-actions', async (request, reply) => {
        try {
            const user = request.user;
    ...
    app.post('/api/v1/ai/pending-actions/:id/confirm', async (request, reply) => {
        try {
            const user = request.user;
    ...
    app.post('/api/v1/ai/pending-actions/:id/reject', async (request, reply) => {
        try {
            const user = request.user;
            const ok = await rejectAction(request.params.id, user.orgId, user.id ?? 'staff');
            if (!ok)
                return reply.status(400).send({ error: 'Không thể từ chối (đã xử lý hoặc không tồn tại)' });
            return { success: true };
        }
        catch (err) {
    ...
    app.post('/api/v1/ai/suggest', async (request, reply) => {
        try {
            ...
            if (!(await assertConversationAccess(request, reply, conversationId)))
                return;
    ...
    app.post('/api/v1/ai/sentiment', async (request, reply) => {
        try {
            const user = request.user;
            const { conversationId } = request.body;
            if (!conversationId)
    ...
    app.post('/api/v1/ai/conversation-analyze', async (request, reply) => {
        try {
            const user = request.user;
            ...
            if (!(await assertConversationAccess(request, reply, conversationId)))
                return;
            const result = await analyzeConversation({ orgId: user.orgId, conversationId }, { forceFresh: !!forceFresh });
            return result;
        }
        catch (err) {
    ...
    app.post('/api/v1/ai/score-lead', async (request, reply) => {
        try {
            const user = request.user;
            const { conversationId } = request.body;
            if (!conversationId)
    ...
    app.post('/api/v1/ai/customer-360', async (request, reply) => {
        const user = request.user;
    ...
    app.post('/api/v1/ai/contact-analyze', async (request, reply) => {
        try {
            const user = request.user;
            ...
            const analysis = await analyzeConversation({ orgId: user.orgId, conversationId: conversation.id }, { forceFresh: !!forceFresh });
            return {
                ...analysis,
    ...
    app.post('/api/v1/ai/api-key/verify', async (request, reply) => {
        try {
            const user = request.user;
            if (!['owner', 'admin'].includes(user.role)) {
    ...
    app.put('/api/v1/ai/api-key', async (request, reply) => {
        try {
            const user = request.user;
            if (!['owner', 'admin'].includes(user.role)) {
                return reply.status(403).send({ error: 'Chỉ admin/owner được quản lý API key' });
    ...
    app.delete('/api/v1/ai/api-key/:provider', async (request, reply) => {
        try {
            const user = request.user;
            if (!['owner', 'admin'].includes(user.role)) {
                return reply.status(403).send({ error: 'Chỉ admin/owner được quản lý API key' });
    ...
    app.get('/api/v1/ai/logic-docs/:type', async (request, reply) => {
        try {
            const user = request.user;
            ...
            const [doc, versions] = await Promise.all([
                getLogicDoc(user.orgId, type),
                getLogicDocVersions(user.orgId, type),
            ]);
            if (!doc)
                return reply.status(404).send({ error: `Logic doc type "${type}" not found. Use POST /seed to create defaults.` });
            return { doc, versions };
        }
        catch (err) {
    ...
    app.put('/api/v1/ai/logic-docs/:type', async (request, reply) => {
        try {
            const user = request.user;
            if (!['owner', 'admin'].includes(user.role)) {
            ...
            const doc = await upsertLogicDoc(user.orgId, type, content, user.id, changeNote);
            return { doc };
        }
        catch (err) {
    ...
    app.post('/api/v1/ai/logic-docs/:type/revert', async (request, reply) => {
        try {
            const user = request.user;
            if (!['owner', 'admin'].includes(user.role)) {
            ...
            const doc = await revertLogicDoc(user.orgId, type, version, user.id);
            return { doc };
        }
        catch (err) {
