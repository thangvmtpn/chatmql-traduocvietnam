...
import { groupByAvatarHash } from './avatar-hash.js';
 ...
 */
function memberScopeFilter(user) {
...
export async function contactRoutes(app) {
    ...
    app.get('/api/v1/contacts', async (request) => {
        ...
        const where = { orgId: user.orgId, mergedInto: null, isGroup: false, deletedAt: null, ...memberScopeFilter(user) };
        if (search) {
            where.OR = [
        ...
        if (lifecycleStage)
            where.lifecycleStage = { in: lifecycleStage.split(',') };
        if (companyId)
            where.companyId = companyId;
        // Member scope wins over the query filter — they can only ever filter inside their own contacts
        if (assignedUserId && user.role !== 'member')
            where.assignedUserId = { in: assignedUserId.split(',') };
    ...
    app.put('/api/v1/contacts/:id', async (request, reply) => {
        ...
        if (body.tags) {
            const oldTags = new Set(Array.isArray(existing.tags) ? existing.tags : []);
        ...
        if (!stageChange)
            emitDomainEvent({ type: 'contact.updated', orgId: user.orgId, id: contact.id });
        return contact;
    });
    ...
    app.post('/api/v1/contacts/:id/convert', async (request, reply) => {
        const user = request.user;
        const toStage = (request.body?.toStage ?? 'customer').trim();
        if (!isValidStage(toStage))
            return badRequest(reply, `Invalid stage: ${toStage}`);
        const existing = await prisma.contact.findFirst({
            where: { id: request.params.id, orgId: user.orgId, ...memberScopeFilter(user) },
            select: { id: true },
        });
    ...
    app.post('/api/v1/contacts/:id/recompute-score', async (request, reply) => {
        const user = request.user;
        const existing = await prisma.contact.findFirst({
            where: { id: request.params.id, orgId: user.orgId, ...memberScopeFilter(user) },
            select: { id: true },
    ...
    app.post('/api/v1/contacts/:id/sync-zalo', async (request, reply) => {
        const user = request.user;
        const contact = await prisma.contact.findFirst({
            where: { id: request.params.id, orgId: user.orgId, ...memberScopeFilter(user) },
        });
        if (!contact)
            return reply.status(404).send({ error: 'Contact not found' });
        if (!contact.zaloUid)
            return reply.status(400).send({ error: 'Contact has no Zalo UID' });
        ...
        try {
            ...
            if (isOa) {
                ...
                if (!profile)
                    return reply.status(400).send({ error: 'Could not fetch profile from Zalo' });
                ...
                if (bestName)
                    updateData.fullName = bestName;
                if (profile.avatar)
                    updateData.avatarUrl = profile.avatar;
                if (profile.phoneNumber && !contact.phone)
                    updateData.phone = profile.phoneNumber;
            }
            ...
            if (updateData.fullName) {
                await prisma.conversation.updateMany({
                    ...
                    data: { displayName: updateData.fullName },
                });
            }
    ...
    app.post('/api/v1/contacts/merge', async (request, reply) => {
        ...
        for (const mergeId of mergeIds)
            emitDomainEvent({ type: 'contact.deleted', orgId: user.orgId, id: mergeId });
    ...
    app.get('/api/v1/contacts/duplicates', async (request) => {
        const user = request.user;
    ...
    app.post('/api/v1/contacts/duplicates/scan', async (request) => {
        ...
        const existingKeys = new Set(existingGroups.map(g => [...g.contactIds].sort().join(',')));
        ...
        const tryCreate = async (ids, matchType, confidence) => {
        ...
        for (const [, group] of hashGroups) {
            ...
            if (await tryCreate(group.map(c => c.id), 'avatar', 0.85))
                created++;
        }
        ...
        for (const [, group] of phoneMap) {
            if (group.length < 2)
                continue;
            group.sort((a, b) => new Date(a.createdAt).getTime() - new Date(b.createdAt).getTime());
            if (await tryCreate(group.map(c => c.id), 'phone', 0.95))
                created++;
        }
        ...
        for (const [, group] of emailMap) {
            if (group.length < 2)
                continue;
            group.sort((a, b) => new Date(a.createdAt).getTime() - new Date(b.createdAt).getTime());
            if (await tryCreate(group.map(c => c.id), 'email', 0.90))
                created++;
        }
    ...
    app.post('/api/v1/contacts/duplicates/:groupId/dismiss', async (request, reply) => {
        const user = request.user;
        const group = await prisma.duplicateGroup.findFirst({
    ...
    app.post('/api/v1/contacts/extract-phones', async (request) => {
        const user = request.user;
        ...
        if (!dryRun && matches.length > 0) {
            ...
            for (let i = 0; i < toApply.length; i += 50) {
                ...
                const results = await prisma.$transaction(batch.map(m => prisma.contact.updateMany({
