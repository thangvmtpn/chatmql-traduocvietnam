...
import { deriveKbLabel } from './kb-service.js';
// ── Constants ─────────────────────────────────────────────────────────────────
...
async function resolveEmbedConfig(orgId) {
...
async function resolveWithProvider(orgId, provider, model) {
    const providerDef = getProviderConfig(provider);
    if (!providerDef)
        return null;
    const apiKey = await getProviderApiKey(orgId, provider);
 ...
 */
export async function embedText(orgId, text) {
    ...
    try {
        ...
        logger.warn({ orgId, err: isAbort ? 'timeout' : err.message }, '[embedding] embedText failed');
        return null;
    }
    finally {
 ...
 */
export async function embedTexts(orgId, texts) {
    ...
    for (let start = 0; start < texts.length; start += EMBED_BATCH_SIZE) {
        ...
        try {
            ...
            const isAbort = err.name === 'AbortError';
            logger.warn({ orgId, err: isAbort ? 'timeout' : err.message }, '[embedding] embedTexts chunk failed');
            // leave chunk null
        }
        finally {
            clearTimeout(timeout);
 ...
 */
export async function storeKbEmbedding(orgId, entryId, vec) {
    const literal = toVectorLiteral(vec);
    // NOTE: literal is computed from the float array (no user input) — injection-safe.
    // orgId and entryId are Prisma parameters.
    await prisma.$executeRaw(Prisma.sql `UPDATE knowledge_entries SET embedding = ${literal}::vector WHERE id = ${entryId} AND org_id = ${orgId}`);
}
 ...
 */
export async function retrieveKbSemantic(orgId, query, topK, keywordFallback, opts) {
...
export async function embedAndStoreKbEntry(orgId, entryId) {
    const entry = await prisma.knowledgeEntry.findFirst({
        where: { id: entryId, orgId },
        select: { title: true, keywords: true, content: true },
    });
    if (!entry)
        return false; // entry deleted — skip silently
 ...
 */
export async function backfillEmbeddings(orgId, opts = {}) {
    const rows = opts.force
        ? await prisma.$queryRaw(Prisma.sql `SELECT id FROM knowledge_entries WHERE org_id = ${orgId} AND status = 'active'`)
        : await prisma.$queryRaw(Prisma.sql `SELECT id FROM knowledge_entries WHERE org_id = ${orgId} AND status = 'active' AND embedding IS NULL`);
    let embedded = 0;
    let failed = 0;
    for (const r of rows) {
        if (await embedAndStoreKbEntry(orgId, r.id))
            embedded++;
        else
            failed++;
    }
