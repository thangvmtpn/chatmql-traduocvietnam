...
export async function libraryRoutes(app) {
    ...
    app.get('/api/v1/library/items', async (request) => {
        ...
        if (kind === 'all' || kind === 'image') {
            ...
            for (const c of cats) {
                const items = c.products.flatMap(p => (p.images || [])
                    .filter(Boolean)
                    .map((img, idx) => ({
                    id: `prod:${p.id}:${idx}`,
                    kind: 'image',
    ...
    app.post('/api/v1/library/send', async (request, reply) => {
        const user = request.user;
        const { conversationId, itemIds } = request.body || {};
        if (!conversationId?.trim())
            return reply.status(400).send({ error: 'Thiếu conversationId' });
        if (!itemIds?.length)
            return reply.status(400).send({ error: 'Chưa chọn tài liệu nào' });
        ...
        for (const raw of itemIds.slice(0, 20)) {
            ...
            try {
                if (prefix === 'prod') {
                    ...
                    if (!img) {
                        skipped.push({ id: raw, reason: 'Không tìm thấy ảnh' });
                        continue;
                    }
                ...
                if (prefix === 'kb') {
                    ...
                    if (!e) {
                        skipped.push({ id: raw, reason: 'Tài liệu chưa được duyệt hoặc đã bị gỡ' });
                        continue;
                    }
        ...
        logger.info({ conversationId: conv.id, userId: user.id, sent: created.length, skipped: skipped.length }, '[library] Nhân viên gửi tài liệu vào hội thoại');
