...
export async function createOrderAndSync(input) {
    ...
    try {
        ...
        let eventChannelAccountId = null;
        ...
        if (eventContactId) {
            ...
            logger.warn({ orderCode: result.order_code, orgId: input.orgId }, '[orders] Không tìm được contact — bỏ qua event order_created (báo cáo Chat → Đơn sẽ thiếu đơn này)');
        }
    }
    catch (err) {
