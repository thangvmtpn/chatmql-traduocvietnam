...
function calculateVipLevel(gmv, aov) {
    ...
    if (aovVal < 500_000)
        aovClass = 'A';
    else if (aovVal < 1_000_000)
        aovClass = 'B';
    else if (aovVal < 2_000_000)
        aovClass = 'C';
    else if (aovVal <= 3_000_000)
        aovClass = 'D';
    else
        aovClass = 'E';
    return `${vip}${aovClass}`;
...
export async function orderRoutes(app) {
    ...
    app.get('/api/v1/orders/customer', async (request, reply) => {
        const phone = request.query.phone?.trim();
        if (!phone)
            return reply.status(400).send({ error: 'Thiếu tham số phone' });
        try {
    ...
    app.get('/api/v1/orders/customer-orders', async (request, reply) => {
        const phone = request.query.phone?.trim();
        if (!phone)
            return reply.status(400).send({ error: 'Thiếu tham số phone' });
        try {
    ...
    function extractPhoneFromName(name) {
        ...
        for (const run of runs) {
            ...
            if (/^0[35789]\d{8}$/.test(run))
                return run;
            // 84xxxxxxxxx — 11 số
            if (/^84[35789]\d{8}$/.test(run))
                return '0' + run.slice(2);
            // xxxxxxxxx — 9 số, thiếu số 0 đầu
            if (/^[35789]\d{8}$/.test(run))
                return '0' + run;
        }
        return '';
    }
    app.get('/api/v1/orders/conversation-context', async (request, reply) => {
        ...
        if (!conv)
            return reply.status(404).send({ error: 'Không tìm thấy hội thoại' });
    ...
    app.get('/api/v1/orders/wards', async (request, reply) => {
    ...
    app.get('/api/v1/orders/catalog', async (request, reply) => {
    ...
    app.get('/api/v1/orders/customer-profile', async (request, reply) => {
        const user = request.user;
        ...
        if (conversationId?.trim()) {
            ...
            if (!phone) {
                phone = extractPhoneFromName(contact?.crmName || contact?.fullName || conv.displayName);
                if (phone) {
                    logger.info({ conversationId: conv.id, phone }, '[orders] Rút số điện thoại từ tên hội thoại để tra hồ sơ CRM');
                }
            }
        }
        if (!phone) {
    ...
    app.post('/api/v1/orders/customer-schedule', async (request, reply) => {
        const b = request.body || {};
        const phone = b.phone?.trim();
        if (!phone)
            return reply.status(400).send({ error: 'Thiếu số điện thoại khách hàng' });
        try {
            ...
            logger.info({ phone, userId: request.user.id }, '[orders] Cập nhật lịch khách hàng');
            return result;
        }
        catch (err) {
    ...
    app.get('/api/v1/orders/customer-activity', async (request, reply) => {
        const user = request.user;
        const conversationId = request.query.conversationId?.trim();
        if (!conversationId)
            return reply.status(400).send({ error: 'Thiếu conversationId' });
        ...
        const want = (t) => !wanted?.length || wanted.includes(t);
        const items = [];
        ...
        for (const m of messages) {
            ...
            if (m.contentType !== 'text' && body.trim().startsWith('{')) {
                try {
                    body = JSON.parse(body).caption || JSON.parse(body).title || `[${m.contentType}]`;
                }
                catch {
                    body = `[${m.contentType}]`;
                }
        ...
        const filtered = q
            ? items.filter(i => i.title.toLowerCase().includes(q) || (i.detail || '').toLowerCase().includes(q))
            : items;
    ...
    app.get('/api/v1/orders/customer-products', async (request, reply) => {
        const phone = request.query.phone?.trim();
        if (!phone)
            return reply.status(400).send({ error: 'Thiếu số điện thoại' });
        try {
            return await fetchCustomerProducts(phone);
        }
        catch (err) {
            return replyCrmError(reply, err, 'lấy sản phẩm đã mua');
        }
    });
    app.get('/api/v1/orders/customer-points', async (request, reply) => {
        const phone = request.query.phone?.trim();
        if (!phone)
            return reply.status(400).send({ error: 'Thiếu số điện thoại' });
        try {
    ...
    app.get('/api/v1/orders/promotions', async (request, reply) => {
        try {
    ...
    app.post('/api/v1/orders/promotions/apply', async (request, reply) => {
        const b = request.body || {};
        if (!b.code?.trim())
            return reply.status(400).send({ error: 'Chưa nhập mã ưu đãi' });
        try {
    ...
    app.get('/api/v1/orders/conversation-library', async (request, reply) => {
        const user = request.user;
        const conversationId = request.query.conversationId?.trim();
        if (!conversationId)
            return reply.status(400).send({ error: 'Thiếu conversationId' });
        ...
        for (const m of rows) {
            ...
            if (kind === 'link') {
                ...
                for (const url of found) {
                    ...
                    catch { /* URL méo — giữ nguyên chuỗi */ }
                    items.push({
    ...
    // ── Tạo đơn ───────────────────────────────────────────────────────
    app.post('/api/v1/orders/create', async (request, reply) => {
        const user = request.user;
        ...
        if (body.conversationId) {
            ...
            if (conv) {
                ...
                logger.warn({ conversationId: body.conversationId, orgId: user.orgId }, '[orders] conversationId không thuộc org — bỏ qua');
            }
        }
    ...
    app.get('/api/v1/orders/:orderCode/status', async (request, reply) => {
        try {
    ...
    app.post('/api/v1/orders/reconcile-fm', async (request, reply) => {
        const user = request.user;
