...
export async function listProducts(orgId, q = {}) {
...
export async function getProduct(orgId, id) {
    const p = await prisma.product.findFirst({ where: { id, orgId }, select: SELECT });
    return p ? serialize(p) : null;
}
export async function createProduct(orgId, data, createdById, opts = {}) {
    if (data.categoryId)
        await assertSameOrgCategory(orgId, data.categoryId);
...
export async function updateProduct(orgId, id, data, opts = {}) {
    ...
    if (d.categoryId !== undefined)
        patch.category = d.categoryId ? { connect: { id: d.categoryId } } : { disconnect: true };
    if (d.code !== undefined)
        patch.code = d.code?.trim() || null;
    if (d.description !== undefined)
        patch.description = d.description;
    if (d.notes !== undefined)
        patch.notes = d.notes;
    if (d.keywords !== undefined)
        patch.keywords = d.keywords;
    if (d.tags !== undefined)
        patch.tags = d.tags;
    if (d.priceType !== undefined)
        patch.priceType = d.priceType;
    if (d.price !== undefined)
        patch.price = d.price != null ? new Prisma.Decimal(d.price) : null;
    if (d.priceMax !== undefined)
        patch.priceMax = d.priceMax != null ? new Prisma.Decimal(d.priceMax) : null;
    if (d.currency !== undefined)
        patch.currency = d.currency;
    if (d.specs !== undefined)
        patch.specs = d.specs;
    if (d.images !== undefined)
        patch.images = d.images;
    if (d.status !== undefined)
        patch.status = d.status;
    if (d.sortOrder !== undefined)
        patch.sortOrder = d.sortOrder;
    try {
...
async function assertSameOrgCategory(orgId, categoryId) {
    ...
    if (!cat)
        throw httpError('Danh mục không hợp lệ', 400);
}
function mapDup(err) {
