...
export function googleSheetCsvUrl(url) {
    ...
    if (!idMatch)
        return url; // assume it's already a direct CSV url
    const id = idMatch[1];
...
export async function fetchSheetRows(url) {
    ...
    if (!res.ok) {
        const e = new Error('Không tải được Google Sheet (kiểm tra quyền chia sẻ công khai)');
        e.statusCode = 400;
        throw e;
    }
...
export async function importProducts(orgId, rows, mapping, opts = {}) {
    ...
    if (opts.categoryId && opts.specMapping && Object.keys(opts.specMapping).length > 0) {
        ...
        const fields = cat?.specSchema?.fields;
    ...
    for (let r = 1; r < rows.length; r++) {
        ...
        try {
            const existing = code
                ...
                : null;
            if (existing) {
                await updateProduct(orgId, existing.id, input, { skipEmbed: true });
                touchedIds.push(existing.id);
                res.updated++;
            }
            else {
                const created = await createProduct(orgId, input, undefined, { skipEmbed: true });
                touchedIds.push(created.id);
                res.created++;
            }
        }
        catch (e) {
            res.errors.push(`Dòng ${r + 1}: ${e.message}`);
        }
    }
    ...
    if (touchedIds.length > 0) {
        await embedAndStoreProductsBatch(orgId, touchedIds).catch(() => { });
    }
    return res;
}
const BOOL_TRUE_RE = /^(true|1|có|yes|x)$/i;
/** Map a row's columns into a typed specs object per the category schema. */
function buildSpecs(row, get, specFields, specMapping) {
    const specs = {};
    if (!specMapping)
        return specs;
...
export async function importKnowledge(orgId, rows, mapping, opts = {}) {
    const res = { created: 0, updated: 0, skipped: 0, errors: [] };
    if (rows.length < 2)
        return res;
...
async function categoryFormat(orgId, categoryId) {
