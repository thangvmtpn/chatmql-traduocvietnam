...
export async function backfillProductEmbeddings(orgId) {
    const rows = await prisma.$queryRaw(Prisma.sql `SELECT id FROM products WHERE org_id = ${orgId} AND status = 'active' AND embedding IS NULL`);
