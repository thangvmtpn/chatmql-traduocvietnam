 ...
 */
export async function getValidTikTokToken(channelAccountId) {
...
export async function sendTikTokImage(channelAccountId, externalThreadId, imageBuffer, fileName = 'image.jpg') {
    ...
    try {
        ...
        const sendRes = await sendTikTokImageMessage(session.shopCipher, externalThreadId, uploadRes.data.image_id, session.accessToken, appKey, appSecret);
        if (sendRes.code === 0) {
            return {
                sent: true,
