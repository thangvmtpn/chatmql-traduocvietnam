...
function verifySignature(rawBody, signature, appSecret) {
    ...
    for (const s of secrets) {
        for (const k of keys) {
            const base = Buffer.concat([Buffer.from(k, 'utf8'), rawBody]);
...
export async function tiktokWebhookRoutes(app) {
     ...
     */
    app.get('/api/v1/tiktok-shop/webhook', async (request, reply) => {
     ...
     */
    app.post('/api/v1/tiktok-shop/webhook', async (request, reply) => {
        const appSecret = process.env.TIKTOK_APP_SECRET;
        const rawBody = request.rawBody;
        const signature = (request.headers['x-tts-signature'] || request.headers['authorization']);
 ...
 */
export async function processTikTokEvent(payload) {
...
//# sourceMappingURL=tiktok-webhook.js.map