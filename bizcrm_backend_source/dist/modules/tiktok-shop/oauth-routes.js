...
import { exchangeCodeForToken, getAuthorizedShops, TIKTOK_AUTH_BASE, } from './tiktok-client.js';
const STATE_TTL_SEC = 600; // 10 minutes
async function getEffectiveAppCreds(orgId, explicitCreds) {
...
export async function tiktokOAuthRoutes(app) {
    /**
     * GET /api/v1/tiktok-shop/config
     * Returns current TikTok Shop configuration (masked secret).
     */
    app.get('/api/v1/tiktok-shop/config', { preHandler: authMiddleware }, async (request, reply) => {
     ...
     */
    app.post('/api/v1/tiktok-shop/config', { preHandler: authMiddleware }, async (request, reply) => {
        const user = request.user;
     ...
     */
    app.get('/api/v1/tiktok-shop/callback', async (request, reply) => {
        ...
        try {
            ...
            const { access_token, refresh_token, access_token_expire_in, open_id, seller_name, } = tokenRes.data;
