...
async function seedRolesForOrg(orgId) {
 ...
 */
async function backfillUserRoles(orgId) {
    const roles = await prisma.role.findMany({
        where: { orgId, systemKey: { not: null } },
        select: { id: true, systemKey: true },
    });
    const byKey = new Map(roles.map((r) => [r.systemKey, r.id]));
    let updated = 0;
...
export async function syncRbac() {
    try {
        ...
        logger.info({ permissions: PERMISSIONS.length, orgs: orgs.length, usersLinked: linked }, '[rbac] danh mục quyền và vai trò gốc đã đồng bộ');
    }
    catch (err) {
