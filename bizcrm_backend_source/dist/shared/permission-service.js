...
const FALLBACK_BY_ROLE = new Map(SYSTEM_ROLES.map((r) => [r.systemKey, new Set(permissionsOf(r))]));
/**
...
async function permissionsOfRoleId(roleId) {
...
export async function userHasPermission(user, key) {
...
export async function getUserPermissionsFresh(userId, fallbackRole) {
    return getUserPermissions(await resolveUserRole(userId, fallbackRole));
}
export async function userHasPermissionFresh(userId, fallbackRole, key) {
