...
export async function ensureVectorIndexes(): Promise<void> {
  ...
  for (const stmt of OTHER_INDEX_STMTS) {
    try {
      ...
      logger.warn({ err: (err as Error).message, stmt }, '[db] index ensure failed')
    }
  }
  try {
    await prisma.$executeRawUnsafe('CREATE EXTENSION IF NOT EXISTS vector')
  } catch (err) {
    logger.warn({ err: (err as Error).message }, '[vector] CREATE EXTENSION vector failed — skipping HNSW index build')
    return
  }
  let ok = 0
  for (const stmt of INDEX_STMTS) {
    try {
      ...
      logger.warn({ err: (err as Error).message, stmt }, '[vector] HNSW index ensure failed (cosine scans still work)')
    }
  }
