 ...
 */
import { recordDebug, type DebugLogEntry } from '../modules/zalo/debug-log.js'
 ...
 */
export function logChannelEvent(input: ChannelEventInput): DebugLogEntry {
  ...
  try {
    if (input.orgId) getIO().to(`org:${input.orgId}:admins`).emit('zalo:debug', entry)
  } catch {
