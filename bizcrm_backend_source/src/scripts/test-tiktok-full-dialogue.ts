...
async function sendWebhook(appSecret: string, shopId: string, convId: string, buyerUid: string, buyerName: string, text: string) {
  const payload = {
    event: 'IM_MESSAGE',
    shop_id: shopId,
    timestamp: Date.now(),
...
async function main() {
  ...
  await sendWebhook(
    ...
    buyerUid,
    buyerName,
    'Dạ vậy chốt cho em 2 túi trà dây tuyết nhé shop. Gửi về: Số 12 Hàng Bài, Hoàn Kiếm, Hà Nội, sđt 0912888999, người nhận là Hằng ạ',
  )
