...
// ZNS (Zalo Notification Service) routes — list approved templates, send.
// Templates: per-OA cache with ~1h TTL. Send: writes ZnsLog audit row,
// optionally appends a Message of contentType='zns_template' if conversationId
// is provided so the send appears in chat history.
...
export async function znsRoutes(app: FastifyInstance): Promise<void> {
  app.addHook('preHandler', authMiddleware);

  // ── GET /api/v1/zns/templates ──────────────────────────────────────────────
  app.get<{ Querystring: { accountId: string; refresh?: string } }>(
  ...
  app.post<{ Body: SendBody }>('/api/v1/zns/send', async (request, reply) => {
    ...
    const log = await prisma.znsLog.create({
      data: {
        ...
        templateId,
        phone: resolvedPhone,
        paramsJson: templateData as any,
        trackingId,
        status: 'pending',
