/**
 * automation-routes.ts — CRUD for automation rules and message templates.
 * Ported from ZaloCRM automation-routes.ts + template-routes.ts.
 */
import type { FastifyInstance } from 'fastify'
import { authMiddleware } from '../auth/auth-middleware.js'
import { prisma } from '../../shared/prisma-client.js'
import { Prisma } from '@prisma/client'
import { renderTemplate, SAMPLE_PREVIEW_CONTEXT, type TemplateRenderContext } from './template-renderer.js'
...
export async function automationRoutes(app: FastifyInstance): Promise<void> {
  ...
  app.post<{ Body: {
    ...
    conditions?: any[]; actions?: any[]; enabled?: boolean; priority?: number;
    flowConfig?: Record<string, any>; flowVersion?: number;
  } }>(
  ...
  app.delete<{ Params: { id: string } }>('/api/v1/automation/rules/:id', async (request, reply) => {
    const user = request.user as { orgId: string; role: string }
    if (!['owner', 'admin'].includes(user.role)) {
      return reply.status(403).send({ error: 'Only admin/owner can manage automation rules' })
    }
  ...
  app.post<{ Params: { id: string }; Body: { contactId?: string; conversationId?: string; dryRun?: boolean; flowConfig?: Record<string, any> } }>(
    '/api/v1/automation/rules/:id/test-run',
    async (request, reply) => {
      const user = request.user as { orgId: string; role: string }
      if (!['owner', 'admin'].includes(user.role)) {
      ...
      if (!flowConfig && id !== '__unsaved__') {
        ...
        ruleName = rule.name
        flowConfig = rule.flowConfig as Record<string, any> | null
      }
      ...
      const ctx: Record<string, any> = { orgId: user.orgId }
      ...
      // ── Path B: contactId provided (contact-based triggers) ──
        ...
        if (conv) ctx.conversationId = conv.id
      ...
      try {
        ...
        const result = await executeFlowV2(effectiveId, ruleName, flowConfig as any, ctx as any, dryRun)
        return {
          ...result,
          testContext: {
  ...
  app.post<{ Body: { name: string; content: string; category?: string; isPersonal?: boolean } }>(
  ...
  app.post<{ Params: { id: string }; Body: { contactId?: string } }>(
    '/api/v1/automation/templates/:id/preview',
    async (request, reply) => {
      ...
      let context: TemplateRenderContext = { ...SAMPLE_PREVIEW_CONTEXT }
      if (contactId) {
  ...
  app.get<{ Querystring: { ruleId?: string; limit?: string } }>(
    ...
    async (request, reply) => {
      ...
      const ruleIds = Array.from(new Set(logs.map((l) => l.entityId).filter(Boolean))) as string[]
