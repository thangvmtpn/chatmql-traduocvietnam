 ...
 */
export async function pollConversationIdle(): Promise<number> {
  let dispatched = 0
  ...
  for (const threshold of IDLE_THRESHOLDS) {
    ...
    for (const conv of conversations) {
      ...
      if (checkpoint && checkpoint.lastCheckedAt >= lastMsg) continue
 ...
 */
export async function resetIdleCheckpoints(conversationId: string): Promise<void> {
 ...
 */
export async function seedIdleCheckpoints(conversationId: string, lastMessageAt: Date): Promise<void> {
  try {
    await Promise.all(IDLE_THRESHOLDS.map((t) =>
      prisma.conversationIdleCheckpoint.upsert({
    ...
    logger.warn({ err: (err as Error).message, conversationId }, '[idle-poller] seed checkpoints failed')
  }
}
