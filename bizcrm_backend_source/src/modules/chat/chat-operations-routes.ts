...
async function resolveMessageRefs(conversationId: string, messageId: string, orgId: string) {
  ...
  if (!message?.externalMsgId) return null
  return {
...
export async function chatOperationsRoutes(app: FastifyInstance) {
  ...
  app.post<{ Params: { id: string; msgId: string } }>(
    '/api/v1/conversations/:id/messages/:msgId/undo',
    chatAccess,
    async (request, reply) => {
      const user = request.user as { orgId: string; id: string }
      ...
      if (!refs) return reply.status(404).send({ error: 'Message not found or has no Zalo message ID' })
      ...
      try {
        ...
        await entry.api.undo(
          { msgId: refs.externalMsgId, cliMsgId: refs.cliMsgId },
          threadId,
          threadType,
        )
  ...
  app.post<{ Params: { id: string }; Body: { stickerId: number; cateId: number; stickerType?: number } }>(
    '/api/v1/conversations/:id/sticker',
    chatAccess,
    async (request, reply) => {
      const user = request.user as { orgId: string; id: string }
      const { id } = request.params
      ...
      try {
        ...
        const result = await entry.api.sendSticker(
          { id: stickerId, cateId, type: stickerType },
          threadId,
          threadType,
        )
  ...
  app.get<{ Params: { cateId: string }; Querystring: { accountId?: string } }>(
    '/api/v1/stickers/category/:cateId',
    { config: { cache: { ttl: 86400 } } },
    async (request, reply) => {
      const user = request.user as { orgId: string }
      const cateId = Number(request.params.cateId)
      if (isNaN(cateId) || cateId < 0) return reply.status(400).send({ error: 'Invalid cateId' })

      const entry = await findConnectedEntry(user.orgId, request.query.accountId)
      if (!entry) return reply.status(503).send({ error: 'No connected Zalo account' })
  ...
  app.get<{ Querystring: { keyword: string; limit?: string; accountId?: string } }>(
    '/api/v1/stickers/search',
    { config: { cache: { ttl: 3600, keyParams: ['keyword', 'limit'] } } },
    async (request, reply) => {
      const user = request.user as { orgId: string }
      const { keyword, limit: limitStr } = request.query
      if (!keyword) return reply.status(400).send({ error: 'keyword is required' })
...
async function findConnectedEntry(orgId: string, accountId?: string) {
  if (accountId) {
    const entry = getPoolEntry(accountId)
    if (entry?.api && entry.status === 'connected') return entry
  }
