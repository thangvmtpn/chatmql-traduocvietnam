/**
 * chat-group-routes.ts — Group info, add member, friends list endpoints.
...
export async function chatGroupRoutes(app: FastifyInstance): Promise<void> {
  // ── Group info (real Zalo API with fallback) ────────────────────────
  app.get<{ Params: { id: string }; Querystring: { refresh?: string } }>('/api/v1/conversations/:id/group-info', { preHandler: requireZaloAccess() }, async (request, reply) => {
    ...
    let members: { uid: string; name: string; avatarUrl: string | null; isAdmin?: boolean }[] = []
        ...
        if (entry?.api) {
          try {
            ...
            if (gridInfo?.currentMems) {
              const adminIds = new Set<string>(gridInfo.adminIds || [])
            ...
            logger.error('[group-info] Zalo API getGroupInfo failed: %s', (err as Error).message)
          }
        }
      }
    }
    ...
    if (members.length === 0) {
      ...
      const senderUids = senders.map(s => s.senderUid).filter(Boolean) as string[]
  ...
  // ── Add member to group ──────────────────────────────────────────────
  app.post<{ Params: { id: string }; Body: { memberIds: string[] } }>('/api/v1/conversations/:id/add-member', { preHandler: requireZaloAccess() }, async (request, reply) => {
    const user = request.user as { orgId: string }
    ...
    try {
      ...
      logger.error('[add-member] Error: %s', (err as Error).message)
      return reply.status(500).send({ error: 'Không thể thêm thành viên: ' + (err as Error).message })
    }
  })

  // ── Get friends list (for add-member picker) ────────────────────────
  app.get<{ Params: { id: string } }>('/api/v1/conversations/:id/friends', { preHandler: requireZaloAccess() }, async (request, reply) => {
    const user = request.user as { orgId: string }
    ...
    try {
      ...
      if (conv.externalThreadId) {
        try {
          const groupInfoResp = await entry.api.getGroupInfo(conv.externalThreadId)
          const gridInfo = groupInfoResp.gridInfoMap?.[conv.externalThreadId]
          if (gridInfo?.memberIds) existingMemberIds = gridInfo.memberIds
        } catch { /* ignore */ }
      }
      ...
      logger.error('[friends] Error: %s', (err as Error).message)
