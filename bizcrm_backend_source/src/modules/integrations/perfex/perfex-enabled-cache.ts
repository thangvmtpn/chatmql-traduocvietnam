...
const enabledOrgs = new Map<string, CacheEntry>()

/** Reload the enabled-org set from the DB. Call at boot and after any config save. */
export async function refreshPerfexEnabledCache(): Promise<void> {
...
export function isPerfexEnabled(orgId: string): boolean {
  return enabledOrgs.has(orgId)
}

export function getEnabledOrgIds(): string[] {
  return [...enabledOrgs.keys()]
}

export function getDebounceSeconds(orgId: string): number {
