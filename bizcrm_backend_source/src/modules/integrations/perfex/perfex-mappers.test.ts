...
import {
  splitName, companyToCustomer, contactToPerfex, contactToLead, payloadHash, resolveContactEmail,
} from './perfex-mappers.js'
import type { PerfexIntegrationConfig } from './perfex-types.js'

const cfg: PerfexIntegrationConfig = {
...
describe('companyToCustomer', () => {
  it('maps name→company + cleans empty fields', () => {
    const c = companyToCustomer({ name: 'Acme', taxCode: null, phone: '0900', website: null, address: null } as Company)
    expect(c.company).toBe('Acme')
    expect(c.phonenumber).toBe('0900')
    expect((c as unknown as Record<string, unknown>).vat).toBeUndefined()
  })
  it('throws on empty company name (Perfex required field)', () => {
    expect(() => companyToCustomer({ name: '  ' } as Company)).toThrow()
  })
})
...
describe('contactToPerfex', () => {
  ...
  it('prefers a real email + drops is_primary when false', () => {
    ...
    expect((p as unknown as Record<string, unknown>).is_primary).toBeUndefined()
  })
})
