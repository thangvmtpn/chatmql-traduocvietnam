...
interface PancakeWebhookPayload {
  event_type?: string       // 'messaging', 'post', etc.
  page_id?: string
  data?: {
    page_id?: string
    conversation?: {
    ...
    message?: {
      ...
      inserted_at?: string     // ISO timestamp "2026-06-13T07:56:29.000000"
      attachments?: Array<{
 ...
 */
export async function processPancakeEvent(
  channelAccountId: string,
  orgId: string,
  payload: PancakeWebhookPayload,
): Promise<void> {
  // ── Extract message data ──────────────────────────────────────────
  const data = payload.data || payload
  const message = (data as any).message
  const conversation = (data as any).conversation
