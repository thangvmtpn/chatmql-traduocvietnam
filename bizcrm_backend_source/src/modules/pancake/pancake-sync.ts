 ...
 */
export async function syncPancakePage(
  orgId: string,
  channelAccountId: string,
  pageId: string,
  pageToken: string,
  direction: SyncDirection = 'pull',
): Promise<{ conversations: number; messages: number; customers: number }> {
...
async function syncConversations(
  ...
  for (let page = 0; page < MAX_PAGES; page++) {
    ...
    for (const conv of conversations) {
      try {
        const synced = await syncSingleConversation(
          orgId, channelAccountId, pageId, pageToken, conv,
        )
        totalConversations++
    ...
    if (conversations.length < 60) break // Last page
  }
...
async function syncSingleConversation(
  ...
  pageId: string,
  pageToken: string,
  pancakeConv: PancakeConversation,
  ...
  const pageCustomer = pancakeConv.page_customer || {} as NonNullable<PancakeConversation['page_customer']>
  ...
  // Enrich existing contact with page_customer data (fill blank fields only)
  {
    ...
    const recentPhones = pancakeConv.recent_phone_numbers || []
    const phoneFromConv = recentPhones[0]?.phone_number || null
    if (!contact.phone && phoneFromConv) updates.phone = phoneFromConv
    // Name
    if (!contact.fullName && senderName) updates.fullName = senderName
    // Notes from page_customer
    if (!contact.notes && pageCustomer.notes) updates.notes = pageCustomer.notes
    // Gender in metadata
  ...
  const convTime = parsePancakeTime(
    pancakeConv.updated_at || pancakeConv.inserted_at
  )
...
async function syncSingleCustomer(
  ...
  if (cust.name) updateData.fullName = cust.name
  if (cust.phone_numbers?.length) updateData.phone = cust.phone_numbers[0]
  if (cust.emails?.length) updateData.email = cust.emails[0]

  if (contact) {
    // Update existing — only fill in blank fields (don't overwrite user edits)
    const fieldsToUpdate: Record<string, any> = {}
    if (!contact.fullName && updateData.fullName) fieldsToUpdate.fullName = updateData.fullName
    if (!contact.phone && updateData.phone) fieldsToUpdate.phone = updateData.phone
    if (!contact.email && updateData.email) fieldsToUpdate.email = updateData.email
