...
import { mapContentType, type FbMessage } from './fb-message-normalize.js';
...
function verifySignature(rawBody: Buffer, sigHeader: string | undefined, appSecret: string): boolean {
...
export async function facebookPageWebhookRoutes(app: FastifyInstance): Promise<void> {
  ...
  // ─── POST: message events ──────────────────────────────────────────────────
  app.post('/api/v1/facebook-page/webhook', async (request: FastifyRequest, reply) => {
    ...
    const body = request.body as FbWebhookBody;
    ...
    for (const entry of body.entry) {
      ...
      for (const event of entry.messaging) {
        await Promise.all(accounts.map((acc) => routeEvent(event, acc.id, acc.orgId).catch((err) =>
          logger.error({ err }, '[fb-webhook] event handler error'),
        )));
      }
    }
  });
...
async function routeEvent(event: FbMessagingEvent, accountId: string, orgId: string): Promise<void> {
...
async function ingestFbMessage(
  accountId: string,
  orgId: string,
  psid: string,
  m: { content: string; contentType: string; msgId: string; timestamp?: number; attachments: any[] },
): Promise<void> {
...
async function enrichFbContact(accountId: string, psid: string, orgId: string): Promise<void> {
  ...
  try {
    ...
    if (needsName && profile.name) data.fullName = profile.name;
    if (needsAvatar && profile.avatar) data.avatarUrl = profile.avatar;
    ...
    if (isTokenExpiredError(err)) await markTokenExpired(accountId);
  }
}
