...
import { FbClient, FbApiError, type FbAttachmentType } from './fb-client.js';

const clientCache = new Map<string, FbClient>();

export async function getClient(accountId: string): Promise<FbClient> {
...
export async function markTokenExpired(accountId: string): Promise<void> {
  clientCache.delete(accountId);
...
export function isFbCsWindowError(errorSubcode: number | undefined): boolean {
...
export async function sendTextViaFb(accountId: string, psid: string, text: string): Promise<FbSendResult> {
  try {
...
export async function sendAttachmentViaFb(
  accountId: string,
  psid: string,
  buffer: Uint8Array,
  filename: string,
