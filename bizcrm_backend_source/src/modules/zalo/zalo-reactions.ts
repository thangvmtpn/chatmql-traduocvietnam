...
const EMOJI_TO_ZALO_REACTION: Record<string, Reactions> = {
  '👍': Reactions.LIKE,
  '❤️': Reactions.HEART,
  '😂': Reactions.HAHA,
  '😮': Reactions.WOW,
...
export const ZALO_REACTION_TO_EMOJI: Record<string, string> = Object.fromEntries(
  Object.entries(EMOJI_TO_ZALO_REACTION).map(([emoji, enumVal]) => [enumVal, emoji]),
)
...
export interface ForwardReactionInput {
  accountId: string
  threadId: string                 // Conversation.externalThreadId
  threadType: 'user' | 'group'
  externalMsgId: string                // Message.externalMsgId (numeric string)
  emoji: string                    // unicode emoji from UI; '' = remove
}

export interface ForwardReactionResult {
  forwarded: boolean
  reason?: string                  // why not forwarded
  error?: string                   // SDK error if thrown
}

/**
 * Best-effort forward to Zalo. Never throws — returns a result for logging.
 */
export async function forwardReactionToZalo(input: ForwardReactionInput): Promise<ForwardReactionResult> {
  const { accountId, threadId, threadType, externalMsgId, emoji } = input

  if (!externalMsgId) return { forwarded: false, reason: 'message has no externalMsgId (local-only)' }
  if (!threadId) return { forwarded: false, reason: 'conversation has no externalThreadId' }
  ...
  if (emoji === '') {
    ...
    if (!mapped) return { forwarded: false, reason: `emoji ${emoji} not in supported map` }
    icon = mapped
  }
