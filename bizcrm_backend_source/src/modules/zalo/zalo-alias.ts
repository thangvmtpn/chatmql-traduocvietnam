...
const rebuildPromises = new Map<string, Promise<Record<string, string>>>()
 ...
 */
export async function resolveZaloAlias(accountId: string, externalUid: string): Promise<string | undefined> {
  const entry = getPoolEntry(accountId)
  if (!entry?.api || typeof entry.api.getAliasList !== 'function') return undefined
  ...
  try {
    ...
    const cached = await redisCache.get<CachedAliasMap>(cacheKey)

    // 2. Fetch the first page to get the `updateTime` (index)
    const firstPage = await entry.api.getAliasList(200, 1)
    if (!firstPage || !firstPage.updateTime) return undefined
