 ...
 */
async function resolveAccountId(request: FastifyRequest): Promise<string | null> {
  ...
  if (params.id) {
    ...
    if (conv) return conv.channelAccountId
  }

  // Body fallback
  const body = request.body as Record<string, string> | undefined
  if (body?.channelAccountId) return body.channelAccountId
 ...
 */
async function resolveManagerAccountIds(userId: string): Promise<Set<string>> {
  ...
  for (const a of ownAccounts) accountIds.add(a.id)
  for (const a of ownAccess) accountIds.add(a.channelAccountId)
  ...
  if (subordinates.length > 0) {
    ...
    for (const a of subAccounts) accountIds.add(a.id)
    for (const a of subAccess) accountIds.add(a.channelAccountId)
  }
...
export function requireZaloAccess() {
  return async function checkZaloAccess(request: FastifyRequest, reply: FastifyReply) {
    const user = request.user as { id: string; orgId: string; role: string } | undefined
    if (!user) {
