...
type AuthUser = { id: string; orgId: string; role: string; via?: string }

function requireUserOwnerAdmin(request: FastifyRequest, reply: FastifyReply): AuthUser | null {
...
export async function apiKeyRoutes(app: FastifyInstance): Promise<void> {
  ...
  app.post('/api/v1/api-keys', async (request, reply) => {
    ...
    if (!name || !name.trim()) return reply.status(400).send({ error: 'Cần tên (name) cho API key' })
    try {
  ...
  app.delete('/api/v1/api-keys/:id', async (request, reply) => {
    ...
    if (!ok) return reply.status(404).send({ error: 'API key không tồn tại hoặc đã thu hồi' })
