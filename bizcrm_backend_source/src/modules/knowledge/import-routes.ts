  ...
  for (const p of paras) {
    if (p.length > CHUNK_MAX) {
      ...
      for (const sent of p.split(/(?<=[.!?…。])\s+/)) {
        if (piece.length + sent.length + 1 > CHUNK_MAX) { chunks.push(piece.trim()); piece = '' }
        piece += (piece ? ' ' : '') + sent
      }
      if (piece.trim()) chunks.push(piece.trim())
      continue
    }
    if (buf.length + p.length + 2 > CHUNK_TARGET && buf) flush()
    buf += (buf ? '\n\n' : '') + p
  }
  flush()
  return chunks
...
// ── Parse tệp ───────────────────────────────────────────────────────────────
async function parseBuffer(filename: string, mimetype: string, buf: Buffer): Promise<string> {
...
async function assertSafeUrl(u: URL): Promise<void> {
  if (!['http:', 'https:'].includes(u.protocol)) throw new Error('Chỉ nhận địa chỉ http/https')
  const host = u.hostname
  if (/^(localhost|.*\.local)$/i.test(host)) throw new Error('Không thu thập địa chỉ nội bộ')
  try {
    const res = await dnsLookup(host, { all: true })
    if (res.some((r) => isPrivateIp(r.address))) throw new Error('Không thu thập địa chỉ nội bộ')
  } catch (e) {
    if ((e as Error).message.includes('nội bộ')) throw e
    throw new Error('Không phân giải được tên miền')
  }
}

async function fetchPage(u: URL): Promise<{ html: string }> {
  ...
  try {
    ...
    if (!res.ok) throw new Error(`Trang trả về ${res.status}`)
    const ct = res.headers.get('content-type') || ''
    if (!ct.includes('text/html') && !ct.includes('text/plain')) throw new Error('Trang không phải HTML/text')
    ...
    for (;;) {
      ...
      if (done) break
      size += value.length
      if (size > WEB_MAX_BYTES) { await reader.cancel(); break }
      parts.push(value)
    }
...
function sameSiteLinks(html: string, base: URL): URL[] {
  const out: URL[] = []
  ...
  for (const m of html.matchAll(/<a[^>]+href=["']([^"'#]+)["']/gi)) {
    try {
      ...
      if (/\.(jpg|jpeg|png|gif|webp|svg|pdf|zip|mp4|css|js|ico)(\?|$)/i.test(u.pathname)) continue
      if (seen.has(u.href) || u.href === base.href) continue
      seen.add(u.href)
      out.push(u)
    } catch { /* bỏ link hỏng */ }
  }
...
// ── Ghi entries ─────────────────────────────────────────────────────────────
async function createPendingEntries(
  orgId: string, userId: string,
  baseTitle: string, chunks: string[], source: string, categoryId?: string,
): Promise<string[]> {
...
export default async function importRoutes(app: FastifyInstance): Promise<void> {
  ...
  app.post('/api/v1/knowledge/import/file', async (request, reply) => {
    ...
    if (!isKbAdmin(user.role)) return sendError(reply, 403, 'Chỉ admin/owner được nạp tài liệu')

    const results: Array<{ file: string; entries: number; truncated: boolean }> = []
    let categoryId: string | undefined
    try {
      const parts = request.parts()
      const files: Array<{ filename: string; mimetype: string; buf: Buffer }> = []
      ...
      if (!files.length) return sendError(reply, 400, 'Chưa đính kèm tệp nào')

      for (const f of files) {
        const text = await parseBuffer(f.filename, f.mimetype, f.buf)
        const chunks = chunkText(text)
        if (!chunks.length) { results.push({ file: f.filename, entries: 0, truncated: false }); continue }
  ...
  app.post<{ Body: { url?: string; depth?: number; categoryId?: string } }>(
    '/api/v1/knowledge/import/web', async (request, reply) => {
      const user = request.user as { id: string; role: string; orgId: string }
      if (!isKbAdmin(user.role)) return sendError(reply, 403, 'Chỉ admin/owner được thu thập website')
      const { url, depth = 0, categoryId } = request.body ?? {}
      if (!url?.trim()) return sendError(reply, 400, 'Thiếu URL')

      let base: URL
      try { base = new URL(url.trim()) } catch { return sendError(reply, 400, 'URL không hợp lệ') }

      try {
        await assertSafeUrl(base)
        const first = await fetchPage(base)
        const queue: Array<{ u: URL; html: string }> = [{ u: base, html: first.html }]
        if (depth >= 1) {
          for (const link of sameSiteLinks(first.html, base).slice(0, WEB_MAX_PAGES - 1)) {
            try { queue.push({ u: link, html: (await fetchPage(link)).html }) }
            catch { /* trang con lỗi thì bỏ qua, không chặn cả đợt */ }
          }
        }
        const results: Array<{ url: string; title: string; entries: number }> = []
        for (const page of queue) {
          const { title, text } = htmlToText(page.html)
          const chunks = chunkText(text)
          if (!chunks.length) { results.push({ url: page.u.href, title, entries: 0 }); continue }
