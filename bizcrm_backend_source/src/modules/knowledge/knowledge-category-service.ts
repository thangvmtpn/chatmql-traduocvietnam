...
function normalizeKind(kind?: string | null): 'knowledge' | 'faq' {
  return kind === 'faq' ? 'faq' : 'knowledge'
}

export async function listKnowledgeCategories(orgId: string) {
...
export type KnowledgeCategoryInput = {
  ...
  kind?: string // knowledge | faq
  importConfig?: unknown
  sortOrder?: number
}

export async function createKnowledgeCategory(orgId: string, data: KnowledgeCategoryInput) {
  if (data.parentId) await assertSameOrg(orgId, data.parentId)
...
export async function updateKnowledgeCategory(orgId: string, id: string, data: Partial<KnowledgeCategoryInput>) {
  const existing = await prisma.knowledgeCategory.findFirst({ where: { id, orgId }, select: { id: true } })
  if (!existing) return null
  if (data.parentId) {
    if (data.parentId === id) throw httpError('Nhóm không thể là cha của chính nó', 400)
  ...
  if (data.kind !== undefined) patch.kind = normalizeKind(data.kind)
...
export async function deleteKnowledgeCategory(orgId: string, id: string): Promise<boolean> {
  const existing = await prisma.knowledgeCategory.findFirst({ where: { id, orgId }, select: { id: true } })
...
export async function seedDefaultKnowledgeCategories(orgId: string): Promise<number> {
  const count = await prisma.knowledgeCategory.count({ where: { orgId } })
  if (count > 0) return 0
  const defaults: Array<{ name: string; kind: 'knowledge' | 'faq' }> = [
...
async function assertSameOrg(orgId: string, id: string) {
  const c = await prisma.knowledgeCategory.findFirst({ where: { id, orgId }, select: { id: true } })
  if (!c) throw httpError('Nhóm cha không hợp lệ', 400)
}
