...
import { getToolsConfig, TOOL_NAMES, type ToolsConfig } from '../tools-config-service.js'
...
// ── Proposal shape extracted from LLM reply ─────────────────────────────
 ...
 */
function escapeControlsInJsonStrings(s: string): string {
  let out = ''
  let inStr = false
  let esc = false
 ...
 */
function parseLenientProposalJson(raw: string): { proposal?: unknown } | null {
  const fenced = raw.replace(/^\s*```(?:json)?/i, '').replace(/```\s*$/, '').trim()
  const tryParse = (s: string): { proposal?: unknown } | null => {
    try { return JSON.parse(s) as { proposal?: unknown } } catch { return null }
  }
 ...
 */
function parseProposalBlock(text: string): ProposalBlock | null {
  ...
  if (parsed) {
    ...
    if (typeof obj.proposedValue === 'string') {
      ...
      for (const [k, v] of Object.entries(obj)) if (!META.has(k)) rest[k] = v
      if (Object.keys(rest).length === 0) return null
      proposedValue = JSON.stringify(rest)
    }
    if (!proposedValue.trim()) return null
    if (typeof obj.rationale !== 'string' || !obj.rationale) return null
    if (typeof obj.targetType !== 'string' || !obj.targetType) return null
    return {
...
// ── System prompt for AI Master ─────────────────────────────────────────
// ── Case context: the specific conversation/reply being improved ─────────
// Loaded from the session's contextConversationId / seed feedback so the Master
// can diagnose the ACTUAL issue (old customer message, old reply, the prompt +
// router decision + KB that produced it) instead of giving generic advice.
export interface CaseContext {
  conversationId: string
  customerName: string | null
  customerMessage: string | null        // the turn that triggered the reply
  flaggedReply: string | null           // the AI reply under review
  feedbackText: string | null
  feedbackCategory: string | null
  transcript: string[]                  // recent conversation lines
  routerDecision: unknown | null        // what the router decided
  responderSystemPrompt: string | null  // the exact prompt the responder used
  kbUsed: string[]                       // KB titles the responder grounded on (pipeline mode)
  toolCalls: Array<{ name: string; result: string }> // function calls the agent made (agent mode)
}
...
export async function loadCaseContext(
  orgId: string,
  opts: { conversationId?: string | null; aiReplyRunId?: string | null; messageId?: string | null },
  ...
  const [conv, recent, traces] = await Promise.all([
    ...
    runId
      ...
      : Promise.resolve([] as Array<{ step: string; payload: unknown }>),
  ])
  ...
  let flaggedReply: string | null = null
  let routerDecision: unknown = null
  let responderSystemPrompt: string | null = null
  let kbUsed: string[] = []
  ...
  for (const t of traces) {
    ...
    if (t.step === 'assemble') {
      if (typeof p.tin_khach === 'string') customerMessage = p.tin_khach
      ...
      if (Array.isArray(p.tri_thuc_KB_dung)) {
        kbUsed = p.tri_thuc_KB_dung
          .map((x: any) => typeof x === 'string' ? x : (x?.tieu_de ? `${x.tieu_de}${x.diem != null ? ` (độ liên quan ${x.diem})` : ''}` : null))
      ...
      if (typeof p.ten_cong_cu === 'string') {
        const scoreNote = Array.isArray(p.ket_qua_diem) && p.ket_qua_diem.length
          ? ' [độ liên quan: ' + p.ket_qua_diem.map((h: any) => `${h.label}=${h.score ?? 'từ khóa'}`).join('; ') + ']'
          : ''
        toolCalls.push({ name: p.ten_cong_cu, result: (typeof p.ket_qua === 'string' ? p.ket_qua : '') + scoreNote })
      }
    } else if (t.step === 'generator') {
      if (typeof p.reply === 'string') flaggedReply = p.reply
      if (typeof p.system_prompt === 'string') responderSystemPrompt = p.system_prompt
    }
  }
...
// ── Org inventory: catalog + KB the Master can reference by real ID ──────
// Lets the Master UPDATE/DELETE existing entries (needs IDs) and avoid creating
// duplicates — instead of being blind to what already exists.
export interface OrgInventory {
  productCategories: Array<{ id: string; name: string }>
  knowledgeCategories: Array<{ id: string; name: string; kind: string }>
  products: Array<{ id: string; name: string }>
  kbEntries: Array<{ id: string; label: string; format: string; categoryId: string | null }>
  scenarios: Array<{ id: string; name: string; description: string; loadMode: string; enabled: boolean }>
}

export async function loadOrgInventory(orgId: string): Promise<OrgInventory> {
...
function buildInventorySection(inv: OrgInventory): string {
...
function buildMasterSystemPrompt(
  ...
  seedFeedbackText?: string,
  caseContext?: CaseContext | null,
  inventory?: OrgInventory | null,
  tools?: ToolsConfig | null,
  retrieval?: { ragTopK: number; ragMinScore: number | null } | null,
): string {
  const logicSection = [
  ...
  if (tools) {
    const names = new Map([
      ...(inventory?.productCategories ?? []).map((c) => [c.id, c.name] as const),
      ...(inventory?.knowledgeCategories ?? []).map((c) => [c.id, c.name] as const),
    ])
    const fmt = (ids: string[]) =>
      ids.length === 0 ? 'TẤT CẢ (không giới hạn)' : ids.map((id) => `${names.get(id) ?? id} (${id})`).join(', ')
    guardrailSection =
 ...
 */
export async function openSession(input: OpenSessionInput): Promise<MasterSessionRow> {
 ...
 */
export async function sendMasterMessage(
  sessionId: string,
  orgId: string,
  userText: string,
): Promise<SendMasterMessageResult> {
  const trimmedText = userText.trim()
  if (!trimmedText) throw new Error('Message cannot be empty')
  if (trimmedText.length > 8000) throw new Error('Message exceeds 8000 characters')
  ...
  const systemPrompt = buildMasterSystemPrompt(
    ...
    inventory,
    toolsCfg,
    { ragTopK: cfg.ragTopK, ragMinScore: cfg.ragMinScore ?? null },
  )
  ...
  if (proposalBlock) {
    const proposal = await createProposal({
      ...
      targetType: proposalBlock.targetType as 'logic_doc' | 'knowledge_entry' | 'thread_memory' | 'prompt_version' | 'product' | 'guardrail',
 ...
 */
export async function closeSession(sessionId: string, orgId: string): Promise<void> {
