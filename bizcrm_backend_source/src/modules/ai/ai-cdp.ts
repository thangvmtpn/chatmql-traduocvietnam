...
export interface AiCdpConfig {
  analysis: {
    messageCount: number          // default 20
    confidenceThreshold: number   // default 0.7
    customPrompt: string          // appended to system prompt
  }
  outputs: {
    lifecycle:  { enabled: boolean; allowDowngrade: boolean }
    leadScore:  { enabled: boolean }
    sentiment:  { enabled: boolean }
    intent:     { enabled: boolean }
    tags:       { enabled: boolean; allowedTags: string[] }
    profile:    { enabled: boolean; fields: string[] }
    customProperties: {
      enabled: boolean
      propertyIds: string[]   // only store IDs — resolve type/options from CDP at runtime
    }
  }
...
interface AiCdpAiResponse {
  confidence: number
  reason: string
  summary?: string
  lifecycle?: { stage: string; reason: string }
  leadScore?: number
  sentiment?: { label: string; confidence: number; reason?: string }
  intent?: string
  tags?: string[]
  painPoints?: string[]
  competitors?: string[]
...
const DEFAULT_CONFIG: AiCdpConfig = {
...
const RICH_ACTION_MAP: Record<string, string> = {
 ...
 */
function extractCustomMsg(obj: any): string | null {
 ...
 */
function formatMessageForTranscript(contentType: string, content: string | null): string | null {
  ...
  switch (contentType) {
    ...
    case 'image': {
      ...
      if (d?.caption || d?.desc) return `[Hình ảnh: ${(d.caption || d.desc).slice(0, 60)}]`
      if (d?.title) return `[Hình ảnh: ${d.title.slice(0, 60)}]`
      return '[Hình ảnh]'
    }
    ...
    case 'video': {
      const d = safeParse(content)
      if (d?.duration) return `[Video: ${formatDuration(d.duration)}]`
      if (d?.title) return `[Video: ${d.title.slice(0, 60)}]`
      return '[Video]'
    }
    ...
    case 'voice': {
      const d = safeParse(content)
      if (d?.duration || d?.dur) return `[Tin nhắn thoại: ${formatDuration(d.duration || d.dur)}]`
    ...
    case 'bank_transfer': {
      const d = safeParse(content)
      if (!d) return '[Chuyển khoản]'
      const label = extractCustomMsg(d)
      if (label) return `[${label}]`
      const parts: string[] = []
      if (d.bankName || d.bankCode) parts.push(d.bankName || d.bankCode)
      if (d.amount) parts.push(`${Number(d.amount).toLocaleString('vi-VN')}đ`)
    ...
    case 'link': {
      ...
      if (d) {
        ...
        if (title) return `[Link: ${title.slice(0, 80)}]`
        if (href) {
          try { return `[Link: ${new URL(href).hostname}]` } catch { /* invalid url */ }
      ...
      if (content.startsWith('http')) {
        try { return `[Link: ${new URL(content).hostname}]` } catch { /* invalid */ }
    ...
    case 'rich': {
      ...
      const customLabel = extractCustomMsg(d)
      if (customLabel) return `[${customLabel.slice(0, 60)}]`

      // 3. Title
      if (d.title) return `[${d.title.slice(0, 60)}]`

      // 4. Action name as fallback
      if (action) return `[Zalo: ${action}]`
...
function buildSystemPrompt(
  cfg: AiCdpConfig,
  currentContact: { lifecycleStage: string; fullName?: string | null; phone?: string | null; email?: string | null; jobTitle?: string | null; source?: string | null; notes?: string | null; tags: string[]; leadScore?: number; aiSentimentLabel?: string | null; aiIntent?: string | null; aiSummary?: string | null; aiPainPoints?: string[]; aiCompetitors?: string[]; aiSignals?: string[] },
  customPropertyDefs: { fieldKey: string; fieldType: string; options: { value: string; label: string }[]; description: string; propertyId: string; currentValue?: string | null }[],
): string {
  const parts: string[] = []
...
5. Các field khác: chỉ trả khi có thay đổi thực sự.`)
  ...
  // Enabled outputs
  const outputFields: string[] = ['"confidence": <0-1>', '"reason": "<nhận định tổng hợp về khách hàng>"']
...
Trả mảng rỗng [] nếu không phát hiện. Mỗi item là 1 tín hiệu ngắn gọn.`)
  ...
  if (cfg.outputs.profile.enabled && cfg.outputs.profile.fields.length > 0) {
    const currentVals = cfg.outputs.profile.fields.map(f => {
      const val = (currentContact as Record<string, any>)[f]
      return `  ${f}: ${val || '(trống)'}`
    }).join('\n')
    parts.push(`\n[PROFILE]
...
export async function runAiCdp(input: AiCdpInput): Promise<{ applied: Record<string, any>; skipped: Record<string, string>; confidence: number; processData?: Record<string, any> }> {
  ...
  const transcript = recentMessages.reverse()
    .map((m: { senderType: string; content: string | null; contentType: string }) => {
      const formatted = formatMessageForTranscript(m.contentType, m.content)
      if (formatted === null) return null // Skip (e.g. sticker)
  ...
  const customPropertyDefs: { fieldKey: string; fieldType: string; options: { value: string; label: string }[]; description: string; propertyId: string; currentValue?: string | null }[] = []
  if (cfg.outputs.customProperties.enabled && cfg.outputs.customProperties.propertyIds.length > 0) {
    ...
    for (const dbProp of dbProps) {
      customPropertyDefs.push({
        ...
        options: (Array.isArray(dbProp.options) ? dbProp.options : []) as { value: string; label: string }[],
  ...
  const systemPrompt = buildSystemPrompt(
    cfg,
    {
      ...
      aiPainPoints: Array.isArray(contact.aiPainPoints) ? (contact.aiPainPoints as string[]) : [],
      aiCompetitors: Array.isArray(contact.aiCompetitors) ? (contact.aiCompetitors as string[]) : [],
      aiSignals: Array.isArray(contact.aiSignals) ? (contact.aiSignals as string[]) : [],
    },
    customPropertyDefs,
  )
...
Phân tích Customer 360° cho khách hàng trên. Đưa ra insights hữu ích cho sales/CSKH.`
  ...
  if (confidence < threshold) {
    ...
    if (Array.isArray(result.painPoints)) {
      const validPainPoints = result.painPoints.filter((p: any) => typeof p === 'string' && p.trim()).map((p: any) => p.trim())
      const currentPP = Array.isArray(contact.aiPainPoints) ? (contact.aiPainPoints as string[]) : []
    ...
    if (Array.isArray(result.competitors)) {
      const validCompetitors = result.competitors.filter((c: any) => typeof c === 'string' && c.trim()).map((c: any) => c.trim())
      const currentComp = Array.isArray(contact.aiCompetitors) ? (contact.aiCompetitors as string[]) : []
    ...
    if (Array.isArray(result.signals)) {
      const validSignals = result.signals.filter((s: any) => typeof s === 'string' && s.trim()).map((s: any) => s.trim())
      const currentSig = Array.isArray(contact.aiSignals) ? (contact.aiSignals as string[]) : []
    ...
    if (cfg.outputs.customProperties.enabled && result.customProperties && typeof result.customProperties === 'object') {
      const propApplied: Record<string, string> = {}
...
function mergeConfig(partial?: Partial<AiCdpConfig>): AiCdpConfig {
