...
export async function runTraceRetention(now = new Date()): Promise<{ orgs: number; deleted: number }> {
...
export function initTraceRetentionCron(): void {
  if (task) return // idempotent
  task = cron.schedule('0 20 * * *', async () => {
    try {
      const r = await runTraceRetention()
      if (r.deleted) logger.info(r, '[ai-trace] đã dọn trace cũ')
    } catch (err) {
