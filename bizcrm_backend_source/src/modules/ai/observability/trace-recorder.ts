/**
 * trace-recorder.ts — Fire-and-forget AiTrace persistence.
 *
 * - async/non-blocking: never throws into caller
 * - fetches traceRetentionDays / traceErrorRetentionDays from AiConfig
 * - redacts PII (phone numbers, emails) before persisting
 * - caps payload to 16 KB (truncates oversized string fields)
 */
...
const configCache = new Map<string, { info: number; error: number; ts: number }>()
...
function redactValue(value: unknown, depth = 0): unknown {
  if (depth > 6) return value // prevent deep recursion on malformed payloads
...
function capPayload(payload: Record<string, unknown>): Record<string, unknown> {
  ...
  if (raw.length <= PAYLOAD_MAX_BYTES) return payload
  ...
  for (const field of TRUNCATE_FIELDS) {
    ...
    if (typeof v === 'string' && v.length > TRUNCATE_FIELD_CHARS) {
      ...
      if (JSON.stringify(capped).length <= PAYLOAD_MAX_BYTES) break
    }
  }
...
async function getRetentionDays(orgId: string): Promise<{ info: number; error: number }> {
 ...
 */
export function recordStep(input: RecordStepInput): void {
...
async function _persistStep(input: RecordStepInput): Promise<void> {
  const level: TraceLevel = input.level ?? 'info'
  ...
  const redacted = redactValue(input.payload) as Record<string, unknown>
