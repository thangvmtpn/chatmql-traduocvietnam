...
export async function deliverSuggestDraft(
  orgId: string,
  convId: string,
  ...
  logger.info(
    { convId, suggestionId: suggestion.id, runId: result.runId },
    '[suggest] AI draft emitted',
  )
}

/** Convenience: run the harness then deliver the draft. */
export async function generateSuggestDraft(
  orgId: string,
  convId: string,
  turnText: string,
): Promise<void> {
