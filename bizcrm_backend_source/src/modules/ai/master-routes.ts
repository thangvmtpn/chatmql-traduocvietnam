...
function sendErr(reply: FastifyReply, err: unknown, status = 500): FastifyReply {
  const msg = err instanceof Error ? err.message : String(err)
  const lower = msg.toLowerCase()
  if (lower.includes('not found')) return reply.status(404).send({ error: msg })
  ...
  if (lower.includes('quota')) return reply.status(429).send({ error: msg })
...
export async function masterRoutes(app: FastifyInstance): Promise<void> {
  ...
  app.post('/api/v1/ai/feedback', async (request, reply) => {
    ...
    try {
      const feedback = await recordFeedback({
        ...
        text: body.text,
        category: body.category as 'tone' | 'accuracy' | 'policy' | 'missing_info' | 'other' | undefined,
      })
  ...
  app.get('/api/v1/ai/feedback', async (request, reply) => {
    ...
    const status = query.status as 'new' | 'reviewed' | 'applied' | 'dismissed' | undefined
  ...
  app.get('/api/v1/ai/master/proposals', async (request, reply) => {
    ...
    const status = query.status as 'pending' | 'approved' | 'rejected' | 'applied' | undefined
    const limit = Math.min(parseInt(query.limit ?? '50', 10) || 50, 200)
    const offset = parseInt(query.offset ?? '0', 10) || 0
