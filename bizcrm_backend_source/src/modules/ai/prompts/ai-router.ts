...
export function buildRouterPrompt(ctx: HarnessContext, opts: { hasTools?: boolean } = {}): string {
  const parts: string[] = []
...
- Chỉ handoff khi cần escalate THẬT: khách đòi gặp người/nhân viên cụ thể, khiếu nại nặng, ngoài phạm vi, thông tin nhạy cảm.`)
  ...
  if (ctx.contact) {
    const c = ctx.contact
    const contactLines: string[] = [`Name: ${c.fullName}`]
    if (c.lifecycleStage) contactLines.push(`Stage: ${c.lifecycleStage}`)
    if (c.aiSentimentLabel) contactLines.push(`Sentiment: ${c.aiSentimentLabel}`)
    if (c.aiIntent) contactLines.push(`Intent: ${c.aiIntent}`)
    if (c.tags.length > 0) contactLines.push(`Tags: ${c.tags.join(', ')}`)
    parts.push(`\n## Customer Profile\n${contactLines.join('\n')}`)
  }
...
const SAFE_FALLBACK: RouterDecision = { shouldReply: false }
...
export function parseRouterDecision(raw: string): RouterDecision {
  try {
    ...
    const intents = Array.isArray(p.intents)
      ? (p.intents as unknown[])
          .filter((i): i is string => typeof i === 'string' && i.length < 80)
          .slice(0, 10)
      : []

    const ragQuery =
      typeof p.ragQuery === 'string' && p.ragQuery.length > 0 && p.ragQuery.length < 300
        ? p.ragQuery
        : undefined

    const needsKnowledge = typeof p.needsKnowledge === 'boolean' ? p.needsKnowledge : false

    let handoff: RouterDecision['handoff']
