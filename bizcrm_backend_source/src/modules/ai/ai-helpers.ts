 ...
 */
function renderNonTextContent(contentType: string): string | null {
  switch (contentType) {
...
export function buildContextBlock(messages: MessageContext[], customerName: string): string {
...
export function parseSentiment(text: string): SentimentResult {
  try {
    const parsed = JSON.parse(text) as Partial<SentimentResult>
    const label = ['positive', 'negative', 'neutral'].includes(parsed.label as string)
      ? (parsed.label as SentimentResult['label'])
      : 'neutral'
 ...
 */
function sanitizeStringArray(raw: unknown, max = 3): string[] {
...
export function parseLeadScore(text: string): LeadScoreResult {
  try {
    const parsed = JSON.parse(text) as Partial<LeadScoreResult>
    const score = Number.isFinite(parsed.score)
      ? Math.max(0, Math.min(100, Math.round(parsed.score as number)))
      : 0
    const intent: LeadScoreResult['intent'] = ['cold', 'warm', 'hot'].includes(parsed.intent as string)
      ? (parsed.intent as LeadScoreResult['intent'])
    ...
    return {
      ...
      painPoints: sanitizeStringArray((parsed as { painPoints?: unknown }).painPoints),
      competitors: sanitizeStringArray((parsed as { competitors?: unknown }).competitors),
    }
  } catch {
