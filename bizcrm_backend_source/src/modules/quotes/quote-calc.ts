...
export function calcLineAmount(quantity: number, unitPrice: number, discountPercent = 0): number {
 ...
 */
export function calcTotals(lines: LineInput[], opts: TotalsInput = {}): Totals {
  const subtotal = round2(
    lines.reduce((sum, l) => sum + calcLineAmount(l.quantity, l.unitPrice, l.discountPercent), 0),
  )
 ...
 */
export function effectiveDiscountPercent(lines: LineInput[], opts: TotalsInput = {}): number {
  const gross = round2(
    lines.reduce((sum, l) => sum + nonNegative(l.quantity) * nonNegative(l.unitPrice), 0),
  )
  if (gross <= 0) return 0
