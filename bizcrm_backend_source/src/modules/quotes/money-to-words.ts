 ...
 */
export function moneyToWords(amount: number, currencyLabel = 'đồng'): string {
  ...
  if (n === 0) return capitalize(`không ${currencyLabel}`)
  ...
  if (triples.length > GROUPS.length) return '' // vượt sức đọc, bỏ qua
