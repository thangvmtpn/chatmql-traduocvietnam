...
function buildMessage(q: {
  number: string; total: number; validUntil: Date | null; type: string
}, url: string): string {
 ...
 */
export async function sendQuoteToContact(
  orgId: string,
  quoteId: string,
  channel: string,
  userId: string,
  ...
  if (channel === 'link') return { channel, delivered: false, url }
  ...
  const text = buildMessage(
    { number: quote.number, total: Number(quote.total), validUntil: quote.validUntil, type: quote.type },
    url,
  )
