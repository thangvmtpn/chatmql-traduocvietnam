...
import {
  listQuotes, getQuote, getQuoteEvents, createQuote, updateQuote, softDeleteQuote,
  markQuoteSent, reviseQuote, duplicateQuote, convertToContract, respondByStaff,
  QuoteNotFoundError, QuoteForbiddenError, QuoteValidationError,
...
} from './quote-template-service.js'
...
function err(reply: FastifyReply, status: number, message: string, code = 'ERROR') {
...
function handle(reply: FastifyReply, e: unknown, log: FastifyInstance['log'], ctx: string) {
  if (e instanceof QuoteNotFoundError || e instanceof TemplateNotFoundError) {
    return err(reply, 404, e.message, 'NOT_FOUND')
  }
  if (e instanceof QuoteForbiddenError) return err(reply, 403, e.message, 'FORBIDDEN')
  if (e instanceof QuoteValidationError) return err(reply, 400, e.message, 'VALIDATION_ERROR')
  if (e instanceof QuoteStateError) return err(reply, 409, e.message, 'INVALID_STATE')
...
export async function quoteRoutes(app: FastifyInstance): Promise<void> {
  ...
  app.get('/api/v1/quote-templates', async (request, reply) => {
    ...
    } catch (e) { return handle(reply, e, app.log, 'list templates') }
  })
  ...
  app.get('/api/v1/quote-templates/:id', async (request, reply) => {
    ...
    } catch (e) { return handle(reply, e, app.log, 'get template') }
  })

  app.post('/api/v1/quote-templates', async (request, reply) => {
    const u = request.user as AuthUser
    if (!canDeleteQuotes(u.role)) return err(reply, 403, 'Không có quyền', 'FORBIDDEN')
    const body = (request.body ?? {}) as any
    if (!body.name) return err(reply, 400, 'Thiếu tên mẫu', 'VALIDATION_ERROR')
    try {
      return reply.status(201).send({ success: true, data: await createTemplate(u.orgId, body) })
    } catch (e) { return handle(reply, e, app.log, 'create template') }
  })
  ...
  app.patch('/api/v1/quote-templates/:id', async (request, reply) => {
    ...
    const { id } = request.params as { id: string }
    try {
      return { success: true, data: await updateTemplate(u.orgId, id, (request.body ?? {}) as any) }
    } catch (e) { return handle(reply, e, app.log, 'update template') }
  })
  ...
  app.delete('/api/v1/quote-templates/:id', async (request, reply) => {
    ...
    try {
      ...
      return { success: true }
    } catch (e) { return handle(reply, e, app.log, 'delete template') }
  })
  ...
  app.get('/api/v1/quotes', async (request, reply) => {
    ...
    } catch (e) { return handle(reply, e, app.log, 'list') }
  })
  ...
  app.get('/api/v1/quotes/export', async (request, reply) => {
    ...
    try {
      const result = await listQuotes(
        u.orgId,
        { id: u.id, role: u.role },
        { status: q.status, type: q.type, contactId: q.contactId, search: q.search, from: q.from, to: q.to },
        { page: 1, limit: 5000 }, // xuất tối đa 5000 dòng
      )
    ...
    } catch (e) { return handle(reply, e, app.log, 'export') }
  })
  ...
  app.get('/api/v1/quotes/:id', async (request, reply) => {
    ...
    } catch (e) { return handle(reply, e, app.log, 'get') }
  })
  ...
  app.get('/api/v1/quotes/:id/events', async (request, reply) => {
    ...
    } catch (e) { return handle(reply, e, app.log, 'events') }
  })

  app.post('/api/v1/quotes', async (request, reply) => {
    const u = request.user as AuthUser
    if (!canManageQuotes(u.role)) return err(reply, 403, 'Không có quyền', 'FORBIDDEN')
    const body = (request.body ?? {}) as any
    if (!body.contactId) return err(reply, 400, 'Thiếu khách hàng', 'VALIDATION_ERROR')
    if (body.type && !QUOTE_TYPES.includes(body.type)) return err(reply, 400, 'Loại chứng từ không hợp lệ', 'VALIDATION_ERROR')
    try {
      const quote = await createQuote(u.orgId, { id: u.id, role: u.role }, body)
      return reply.status(201).send({ success: true, data: quote })
    } catch (e) { return handle(reply, e, app.log, 'create') }
  })
  ...
  app.patch('/api/v1/quotes/:id', async (request, reply) => {
    ...
    const { id } = request.params as { id: string }
    try {
      return { success: true, data: await updateQuote(u.orgId, id, { id: u.id, role: u.role }, (request.body ?? {}) as any) }
    } catch (e) { return handle(reply, e, app.log, 'update') }
  })
  ...
  app.delete('/api/v1/quotes/:id', async (request, reply) => {
    ...
    try {
      ...
      return { success: true }
    } catch (e) { return handle(reply, e, app.log, 'delete') }
  })
  ...
  app.post('/api/v1/quotes/:id/send', async (request, reply) => {
    ...
    const channel = body.channel && SEND_CHANNELS.includes(body.channel as any) ? body.channel : 'link'
    try {
    ...
    } catch (e) { return handle(reply, e, app.log, 'send') }
  })
  ...
  app.post('/api/v1/quotes/:id/duplicate', async (request, reply) => {
    ...
    try {
      ...
      return reply.status(201).send({ success: true, data: quote })
    } catch (e) { return handle(reply, e, app.log, 'duplicate') }
  })
  ...
  app.post('/api/v1/quotes/:id/revise', async (request, reply) => {
    ...
    } catch (e) { return handle(reply, e, app.log, 'revise') }
  })
  ...
  app.post('/api/v1/quotes/:id/convert-to-contract', async (request, reply) => {
    ...
    } catch (e) { return handle(reply, e, app.log, 'convert') }
  })
  ...
  app.post('/api/v1/quotes/:id/respond', async (request, reply) => {
    ...
    } catch (e) { return handle(reply, e, app.log, 'respond') }
  })
}
