...
type QuoteRow = {
  id: string; orgId: string; number: string; total: unknown; type: string; status: string
  contactId: string; assignedUserId: string | null; createdById: string | null
  contact: { id: string; lifecycleStage: string; fullName: string | null; crmName: string | null } | null
}

async function loadQuote(id: string, orgId: string): Promise<QuoteRow | null> {
...
async function trackCdp(event: DomainEvent, quote: QuoteRow): Promise<void> {
...
async function logActivity(event: DomainEvent, quote: QuoteRow): Promise<void> {
...
async function notifyOwner(event: DomainEvent, quote: QuoteRow): Promise<void> {
...
async function handleQuoteEvent(event: DomainEvent): Promise<void> {
...
export function registerQuoteSubscribers(): () => void {
  if (unsubscribe) return unsubscribe

  unsubscribe = onDomainEvent((event) => {
    if (!QUOTE_EVENTS.has(event.type)) return
