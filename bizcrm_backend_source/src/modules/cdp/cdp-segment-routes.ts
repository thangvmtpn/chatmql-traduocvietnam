...
interface SegmentCondition {
  type: 'property' | 'event' | 'contact' | 'lifecycle'
  field: string            // fieldKey for properties, eventName for events, column name for contact
  operator: string         // equals, not_equals, contains, gt, lt, gte, lte, is, is_not, before, after, has_event, event_count_gte
  value: string | number
}
...
async function evaluateSegment(orgId: string, conditions: SegmentConditionGroup[]): Promise<string[]> {
  ...
  for (const group of conditions) {
    ...
    for (const cond of group.conditions) {
      ...
      if (cond.type === 'contact') {
        ...
        const where: any = { orgId, isGroup: false, deletedAt: null, mergedInto: null }
        const field = cond.field // status, source, leadScore, etc.

        switch (cond.operator) {
          case 'equals': where[field] = String(cond.value); break
          case 'not_equals': where[field] = { not: String(cond.value) }; break
          case 'contains': where[field] = { contains: String(cond.value), mode: 'insensitive' }; break
          case 'gt': where[field] = { gt: Number(cond.value) }; break
          case 'lt': where[field] = { lt: Number(cond.value) }; break
          case 'gte': where[field] = { gte: Number(cond.value) }; break
          case 'lte': where[field] = { lte: Number(cond.value) }; break
          case 'is_null': where[field] = null; break
          case 'is_not_null': where[field] = { not: null }; break
        }
        ...
        const where: any = { orgId, propertyId: prop.id }

        switch (cond.operator) {
          case 'equals': where.value = String(cond.value); break
          case 'not_equals': where.value = { not: String(cond.value) }; break
          case 'contains': where.value = { contains: String(cond.value), mode: 'insensitive' }; break
          case 'gt': case 'gte': case 'lt': case 'lte':
            ...
            ids = allVals
              .filter(v => {
                ...
                if (cond.operator === 'gt') return n > numVal
                if (cond.operator === 'gte') return n >= numVal
                if (cond.operator === 'lt') return n < numVal
                if (cond.operator === 'lte') return n <= numVal
                return false
              })
              .map(v => v.contactId)
            break
    ...
    if (contactIds === null) {
      ...
      const set: Set<string> = new Set(contactIds)
...
export async function cdpSegmentRoutes(app: FastifyInstance): Promise<void> {
  ...
  }>('/api/v1/cdp/segments', async (request, reply) => {
    const user = request.user as { orgId: string }
    const { name, description, conditions } = request.body

    if (!name?.trim()) return badRequest(reply, 'name is required')
    if (!conditions?.length) return badRequest(reply, 'conditions are required')
  ...
  app.put<{
  ...
  }>('/api/v1/cdp/segments/:id', async (request, reply) => {
    const user = request.user as { orgId: string }
    ...
    if (!existing) return notFound(reply, 'Segment')
  ...
  // ── DELETE segment ──────────────────────────────────────────────
  app.delete<{ Params: { id: string } }>('/api/v1/cdp/segments/:id', async (request, reply) => {
    const user = request.user as { orgId: string }
    const { id } = request.params
  ...
  // ── RECALCULATE segment ─────────────────────────────────────────
  app.post<{ Params: { id: string } }>('/api/v1/cdp/segments/:id/calculate', async (request, reply) => {
    const user = request.user as { orgId: string }
    const { id } = request.params
  ...
  }>('/api/v1/cdp/segments/:id/contacts', async (request, reply) => {
    const user = request.user as { orgId: string }
    const { id } = request.params
