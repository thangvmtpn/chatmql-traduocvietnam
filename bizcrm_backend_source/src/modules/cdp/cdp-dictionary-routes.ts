...
export const cdpDictionaryRoutes: FastifyPluginAsync = async (app) => {
  ...
  app.post('/api/v1/cdp/dictionary', async (req, reply) => {
    const orgId = req.user.orgId
    const { eventName, displayName, description, schema, isActive } = req.body as any
  ...
  app.put('/api/v1/cdp/dictionary/:id', async (req, reply) => {
    ...
    const { displayName, description, schema, isActive } = req.body as any
