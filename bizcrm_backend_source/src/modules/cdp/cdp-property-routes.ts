...
export async function cdpPropertyRoutes(app: FastifyInstance): Promise<void> {
  app.addHook('preHandler', authMiddleware)

  // ── LIST custom properties ──────────────────────────────────────
  app.get<{
    Querystring: { group?: string }
  }>('/api/v1/cdp/properties', async (request) => {
  ...
  }>('/api/v1/cdp/properties', async (request, reply) => {
    const user = request.user as { orgId: string }
  ...
  app.put<{
  ...
  }>('/api/v1/cdp/properties/:id', async (request, reply) => {
    const user = request.user as { orgId: string }
    ...
    if (!existing) return notFound(reply, 'Property')
  ...
  }>('/api/v1/contacts/:contactId/properties', async (request, reply) => {
    const user = request.user as { orgId: string }
  ...
  app.put<{
    ...
    const results = await Promise.all(
      values.map(v =>
        prisma.contactPropertyValue.upsert({
          where: {
  ...
  }>('/api/v1/cdp/property-groups/rename', async (request, reply) => {
    const user = request.user as { orgId: string }
    const { oldName, newName } = request.body
    if (!oldName || !newName?.trim()) return badRequest(reply, 'oldName and newName required')
  ...
  app.put<{
    Body: { items: { id: string; groupName?: string; sortOrder: number }[] }
  }>('/api/v1/cdp/property-groups/reorder', async (request, reply) => {
    const user = request.user as { orgId: string }
    const { items } = request.body
    if (!Array.isArray(items)) return badRequest(reply, 'items array required')
    await Promise.all(items.map(item =>
      prisma.customProperty.updateMany({
