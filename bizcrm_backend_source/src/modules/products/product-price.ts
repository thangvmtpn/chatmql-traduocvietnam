 ...
 */
export function formatProductPrice(p: PriceInfo): string {
  switch (p.priceType) {
    case 'free':
      return 'Miễn phí'
    case 'contact':
    ...
    case 'range':
      if (p.price != null && p.priceMax != null) return `${vnd(p.price)} – ${vnd(p.priceMax)} ${p.currency}`
      if (p.price != null) return `Từ ${vnd(p.price)} ${p.currency}`
      return 'Liên hệ báo giá'
    case 'fixed':
    default:
