...
import {
  listProductCategories, getProductCategory, createProductCategory,
...
export async function productRoutes(app: FastifyInstance): Promise<void> {
  ...
  app.post('/api/v1/products/upload-image', async (request, reply) => {
    ...
    if (!file) return err(reply, 400, 'Vui lòng chọn file ảnh')
    if (!ALLOWED_IMG_MIMES.has(file.mimetype)) return err(reply, 400, 'Chỉ chấp nhận ảnh JPG, PNG, WebP, GIF')
    const buffer = await file.toBuffer()
    if (buffer.length > MAX_IMG_SIZE) return err(reply, 400, 'Ảnh tối đa 5MB')
  ...
  app.get('/api/v1/product-categories', async (request, reply) => {
    ...
    } catch (e: any) { app.log.error({ e }, '[products] list cats'); return err(reply, 500, 'Lỗi tải danh mục') }
  })
  ...
  app.post('/api/v1/product-categories', async (request, reply) => {
    ...
    if (!body.name) return err(reply, 400, 'Thiếu tên danh mục')
    try {
      const cat = await createProductCategory(u.orgId, body)
      return reply.status(201).send({ success: true, data: cat })
    } catch (e: any) { return err(reply, e.statusCode ?? 500, e.message ?? 'Lỗi tạo danh mục', e.code) }
  })

  app.patch<{ Params: { id: string }; Body: any }>('/api/v1/product-categories/:id', async (request, reply) => {
    const u = request.user as AuthUser
    if (!canManage(u.role)) return err(reply, 403, 'Không có quyền', 'FORBIDDEN')
    try {
      const cat = await updateProductCategory(u.orgId, request.params.id, request.body ?? {})
      if (!cat) return err(reply, 404, 'Không tìm thấy danh mục')
      return { success: true, data: cat }
    } catch (e: any) { return err(reply, e.statusCode ?? 500, e.message ?? 'Lỗi cập nhật', e.code) }
  })

  app.delete<{ Params: { id: string } }>('/api/v1/product-categories/:id', async (request, reply) => {
    const u = request.user as AuthUser
    if (!canManage(u.role)) return err(reply, 403, 'Không có quyền', 'FORBIDDEN')
    const ok = await deleteProductCategory(u.orgId, request.params.id)
    if (!ok) return err(reply, 404, 'Không tìm thấy danh mục')
  ...
  app.get('/api/v1/products', async (request, reply) => {
    ...
    } catch (e: any) { app.log.error({ e }, '[products] list'); return err(reply, 500, 'Lỗi tải sản phẩm') }
  })

  app.get<{ Params: { id: string } }>('/api/v1/products/:id', async (request, reply) => {
    const u = request.user as AuthUser
    const p = await getProduct(u.orgId, request.params.id)
    if (!p) return err(reply, 404, 'Không tìm thấy sản phẩm')
  ...
  app.post('/api/v1/products', async (request, reply) => {
    ...
    if (!canManage(u.role)) return err(reply, 403, 'Không có quyền', 'FORBIDDEN')
    const body = (request.body ?? {}) as any
    if (!body.name) return err(reply, 400, 'Thiếu tên sản phẩm')
    try {
      const p = await createProduct(u.orgId, body, u.id)
      return reply.status(201).send({ success: true, data: p })
    } catch (e: any) { return err(reply, e.statusCode ?? 500, e.message ?? 'Lỗi tạo sản phẩm', e.code) }
  })

  app.patch<{ Params: { id: string }; Body: any }>('/api/v1/products/:id', async (request, reply) => {
    const u = request.user as AuthUser
    if (!canManage(u.role)) return err(reply, 403, 'Không có quyền', 'FORBIDDEN')
    try {
  ...
  app.delete<{ Params: { id: string } }>('/api/v1/products/:id', async (request, reply) => {
    const u = request.user as AuthUser
    if (!canManage(u.role)) return err(reply, 403, 'Không có quyền', 'FORBIDDEN')
    const ok = await deleteProduct(u.orgId, request.params.id)
    if (!ok) return err(reply, 404, 'Không tìm thấy sản phẩm')
    return { success: true, data: { deleted: true } }
  })
  ...
  app.post('/api/v1/product-knowledge/query', async (request, reply) => {
    try {
      ...
      if (mode === 'aggregate') return { success: true, data: await aggregate(u.orgId) }
      if (mode === 'filter') {
      ...
      // semantic
      if (!b.query?.trim()) return err(reply, 400, 'query là bắt buộc cho mode semantic')
      return { success: true, data: await semanticSearch(u.orgId, b.query, {
        scope: b.scope, topK: b.topK, categoryId: b.categoryId,
      }) }
    } catch (e: any) { app.log.error({ e }, '[pkb] query'); return err(reply, 500, 'Lỗi truy vấn') }
  })
  ...
  app.post('/api/v1/product-knowledge/import', async (request, reply) => {
    ...
    if (!b.sheetUrl && !b.csv) return err(reply, 400, 'Cần sheetUrl hoặc csv')
    try {
      ...
      return { success: true, data: result }
    } catch (e: any) { return err(reply, e.statusCode ?? 500, e.message ?? 'Lỗi import', e.code) }
  })
}
