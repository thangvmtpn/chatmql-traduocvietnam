...
export async function semanticSearch(
  orgId: string,
  query: string,
  opts: { scope?: QueryScope; topK?: number; categoryId?: string } = {},
) {
  const topK = clamp(opts.topK ?? 5, 1, 20)
  const scope = opts.scope ?? 'all'
  const out: { products?: unknown[]; knowledge?: unknown[] } = {}
...
export async function structuredFilter(
  orgId: string,
  opts: {
    scope?: QueryScope; categoryId?: string; productId?: string
...
export async function aggregate(orgId: string) {
