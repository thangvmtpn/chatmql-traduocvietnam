...
import {
  buildFunnel, winRate, avgDaysToClose, rankUsers, fillPeriods, type StatusCounts,
} from './quote-analytics-calc.js'

const counts = (p: Partial<StatusCounts> = {}): StatusCounts => ({
...
describe('ranh giới multi-tenant — báo cáo báo giá', () => {
  ...
  it('mọi truy vấn Prisma trong service đều có orgId', () => {
    ...
    const whereConsts = new Set<string>()
    for (const m of svc.matchAll(/const (\w+): Prisma\.\w+WhereInput = \{([\s\S]*?)\n  \}/g)) {
      if (/orgId/.test(m[2])) whereConsts.add(m[1])
    }
    ...
    for (const s of stmts) {
      ...
      const viaConst = [...whereConsts].some((c) =>
        new RegExp(`where:\\s*\\{?\\s*(\\.\\.\\.)?${c}\\b`).test(head))
...
describe('fillPeriods', () => {
  it('chèn ngày trống để biểu đồ không đứt đoạn', () => {
    const r = fillPeriods(
      [{ period: '2026-07-03', count: 2, value: 500 }],
