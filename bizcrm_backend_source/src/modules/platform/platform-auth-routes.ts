...
import {
  PLATFORM_ACCESS_TOKEN_EXPIRY,
  createPlatformRefreshToken,
  rotatePlatformRefreshToken,
  revokeAllPlatformTokens,
  cleanupExpiredPlatformTokens,
  PlatformTokenError,
} from './platform-token-service.js'
...
export async function platformAuthRoutes(app: FastifyInstance): Promise<void> {
  ...
  async function issueTokenPair(payload: { kind: 'platform'; sub: string; email: string; fullName: string }) {
    // Platform tokens carry `sub` instead of company id/role/orgId — sign via cast.
    const accessToken = app.jwt.sign(payload as never, { expiresIn: PLATFORM_ACCESS_TOKEN_EXPIRY })
  ...
  app.post<{ Body: { fullName: string; email: string; password: string } }>(
    '/api/v1/platform/setup',
    async (request, reply) => {
  ...
  app.post<{ Body: { email: string; password: string } }>(
    ...
    async (request, reply) => {
      ...
      if (!email || !password) return reply.status(400).send({ error: 'Thiếu email hoặc mật khẩu' })
  ...
  app.post<{ Body: { refreshToken: string } }>(
    ...
    async (request, reply) => {
      ...
      try {
        ...
        const payload = { kind: 'platform' as const, sub: admin.id, email: admin.email, fullName: admin.fullName }
        const accessToken = app.jwt.sign(payload as never, { expiresIn: PLATFORM_ACCESS_TOKEN_EXPIRY })
  ...
  setInterval(async () => {
    try {
      ...
      if (count > 0) app.log.info(`[platform-auth] Cleaned up ${count} expired refresh tokens`)
    } catch (err) {
