...
export async function platformOrgRoutes(app: FastifyInstance): Promise<void> {
  ...
  app.get<{ Params: { orgId: string } }>('/api/v1/platform/orgs/:orgId', async (request, reply) => {
  ...
  app.patch<{ Params: { orgId: string }; Body: { name?: string; plan?: string | null; adminNotes?: string | null } }>(
    ...
    async (request, reply) => {
      ...
      if (Object.keys(data).length === 0) return reply.status(400).send({ error: 'Không có thay đổi' })
      const exists = await prisma.organization.findUnique({ where: { id: request.params.orgId }, select: { id: true } })
      if (!exists) return reply.status(404).send({ error: 'Organization not found' })
  ...
  app.patch<{ Params: { orgId: string }; Body: { expiresAt?: string | null; status?: string } }>(
    '/api/v1/platform/orgs/:orgId/license',
    async (request, reply) => {
