...
export async function platformReportRoutes(app: FastifyInstance): Promise<void> {
  ...
  app.get('/api/v1/platform/reports/overview', async () => {
    ...
    for (const o of orgs) {
      ...
      if (ds === 'suspended') suspended++
      else if (ds === 'expired') expired++
      else if (ds === 'unlimited') unlimited++
      else active++
      if (isExpiringWithin(o, 7, now)) expiringIn7d++
      if (o.createdAt >= monthAgo) newLast30d++
    }
