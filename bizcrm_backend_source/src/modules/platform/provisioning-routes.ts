...
export async function platformProvisioningRoutes(app: FastifyInstance): Promise<void> {
  app.addHook('preHandler', requirePlatformAdmin)

  // POST /api/v1/platform/orgs — create company + initial owner
  app.post<{
    Body: {
      orgName: string; ownerFullName: string; ownerEmail: string; ownerPassword: string
      expiresAt?: string | null; plan?: string | null
    }
  }>('/api/v1/platform/orgs', async (request, reply) => {
    const { orgName, ownerFullName, ownerEmail, ownerPassword, expiresAt, plan } = request.body ?? ({} as any)
    if (!orgName || !ownerFullName || !ownerEmail || !ownerPassword) {
      return reply.status(400).send({ error: 'Thiếu thông tin bắt buộc' })
    }
    if (ownerPassword.length < 8) return reply.status(400).send({ error: 'Mật khẩu tối thiểu 8 ký tự' })
    const email = ownerEmail.toLowerCase().trim()
    const dup = await prisma.user.findUnique({ where: { email }, select: { id: true } })
    if (dup) return reply.status(409).send({ error: 'Email đã được sử dụng', code: 'EMAIL_TAKEN' })

    let expires: Date | null = null
    if (expiresAt) {
      const d = new Date(expiresAt)
      if (isNaN(d.getTime())) return reply.status(400).send({ error: 'expiresAt không hợp lệ' })
      expires = d
    }
  ...
  app.get<{ Params: { orgId: string } }>('/api/v1/platform/orgs/:orgId/users', async (request, reply) => {
  ...
  app.post<{ Params: { orgId: string }; Body: { fullName: string; email: string; password: string; role?: string } }>(
    '/api/v1/platform/orgs/:orgId/users',
    async (request, reply) => {
      const { fullName, email, password, role } = request.body ?? ({} as any)
      if (!fullName || !email || !password) return reply.status(400).send({ error: 'Thiếu thông tin bắt buộc' })
      if (password.length < 8) return reply.status(400).send({ error: 'Mật khẩu tối thiểu 8 ký tự' })
  ...
  app.post<{ Params: { orgId: string; userId: string }; Body: { newPassword: string } }>(
    '/api/v1/platform/orgs/:orgId/users/:userId/reset-password',
    async (request, reply) => {
      const { newPassword } = request.body ?? ({} as any)
      if (!newPassword || newPassword.length < 8) return reply.status(400).send({ error: 'Mật khẩu tối thiểu 8 ký tự' })
      const user = await prisma.user.findFirst({
        where: { id: request.params.userId, orgId: request.params.orgId }, select: { id: true },
      })
      if (!user) return reply.status(404).send({ error: 'User not found in org' })
  ...
  app.patch<{ Params: { orgId: string; userId: string }; Body: { isActive?: boolean } }>(
    '/api/v1/platform/orgs/:orgId/users/:userId',
    async (request, reply) => {
      const { isActive } = request.body ?? {}
      if (typeof isActive !== 'boolean') return reply.status(400).send({ error: 'isActive phải là boolean' })
      const user = await prisma.user.findFirst({
        where: { id: request.params.userId, orgId: request.params.orgId }, select: { id: true },
      })
      if (!user) return reply.status(404).send({ error: 'User not found in org' })
