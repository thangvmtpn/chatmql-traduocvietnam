...
export async function createPlatformRefreshToken(adminId: string, family?: string): Promise<string> {
...
export async function rotatePlatformRefreshToken(oldToken: string): Promise<{ newToken: string; adminId: string }> {
  const existing = await prisma.platformRefreshToken.findUnique({ where: { token: oldToken } })
  if (!existing) throw new PlatformTokenError('TOKEN_INVALID', 'Refresh token not found')
...
export async function revokeAllPlatformTokens(adminId: string): Promise<void> {
  await prisma.platformRefreshToken.deleteMany({ where: { adminId } })
}

/** Clean up expired tokens. */
export async function cleanupExpiredPlatformTokens(): Promise<number> {
