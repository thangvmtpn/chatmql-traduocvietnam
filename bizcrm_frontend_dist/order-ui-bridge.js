...
(function () {
  ...
  function injectOrderButtons() {
    // 1. Inject in Chat Toolbar (next to AI Gợi ý / Tin nhanh)
    ...
    if (!existingBtn) {
      ...
      if (btnGroup) {
        ...
        orderBtn.onclick = () => window.openChatMqlOrderModal();
        btnGroup.appendChild(orderBtn);
      }
    }

    // 2. Inject in Customer 360 Profile Sidebar
    ...
    if (profileSidebar && !existingSidebarBtn) {
      ...
      orderBtnSidebar.onclick = () => window.openChatMqlOrderModal();
